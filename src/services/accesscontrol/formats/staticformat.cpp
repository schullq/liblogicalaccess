...
unsigned char StaticFormat::calculateParity(const void *data, size_t dataLengthBytes,
                                            ParityType parityType, size_t start,
                                            size_t parityLengthBits)
{
    unsigned char parity = 0x00;
    for (size_t i = start; i < (start + parityLengthBits) && i < (dataLengthBytes * 8);
         i++)
    {
        parity = (unsigned char)((parity & 0x01) ^
                                 ((unsigned char)(reinterpret_cast<const char *>(
                                                      data)[i / 8] >>
                                                  (7 - (i % 8))) &
                                  0x01));
    }
...
size_t StaticFormat::getEncodingLinearData(void *data, size_t dataLengthBytes) const
{
    if (data != NULL)
    {
        EXCEPTION_ASSERT_WITH_LOG(dataLengthBytes >= 2, std::invalid_argument,
                                  "dataLengthBytes must be at least 2");

        if (data != NULL)
        {
            reinterpret_cast<char *>(data)[0] =
                static_cast<char>(d_dataRepresentation->getType());
    ...
    if (data != NULL)
    {
        d_dataRepresentation.reset(
            DataRepresentation::getByEncodingType(static_cast<EncodingType>(
                reinterpret_cast<const char *>(data)[(*indexByte)++])));
        d_dataType.reset(DataType::getByEncodingType(static_cast<EncodingType>(
            reinterpret_cast<const char *>(data)[(*indexByte)++])));
    }
}
...
void StaticFormat::convertField(void *data, size_t dataLengthBytes, unsigned int *pos,
                                unsigned long long field, unsigned int fieldlen) const
{
    ...
    if (convertedDataTypeLengthBits > 0)
    {
            ...
            convertedDataTypeLengthBits)
        {
            ...
            if (convertedDataRepresentationLengthBits > 0)
            {
                ...
                memset(convertedDataRepresentationData, 0x00,
...
unsigned long long StaticFormat::revertField(const void *data, size_t dataLengthBytes,
                                             unsigned int *pos,
                                             unsigned int fieldlen) const
{
    ...
    if ((tmp = BitHelper::extract(extractData, extractedSizeBytes, data, dataLengthBytes,
                                  getDataLength(), *pos, extractedSizeBits)) > 0)
    {
...
size_t StaticFormat::getSkeletonLinearData(void *data, size_t dataLengthBytes) const
{
    ...
    if (data != NULL)
    {
        ...
        index += getEncodingLinearData(reinterpret_cast<char *>(data) + length,
