...
size_t Getronik40BitFormat::getFormatLinearData(void * /*data*/,
                                                size_t /*dataLengthBytes*/) const
{
    return 0;
}

void Getronik40BitFormat::setFormatLinearData(const void * /*data*/,
                                              size_t * /*indexByte*/)
{
    // DOES NOTHING
}
...
unsigned char Getronik40BitFormat::getRightParity(const void *data,
                                                  size_t dataLengthBytes) const
{
    ...
    {
        parity =
            calculateParity(data, dataLengthBytes, PT_EVEN, static_cast<size_t>(0), 38);
        size_t pos = 39;
        parity     = parity ^
                 (unsigned char)((unsigned char)(reinterpret_cast<const unsigned char *>(
                                                     data)[pos / 8]
                                                 << (pos % 8)) >>
                                 7);
    }
...
void Getronik40BitFormat::getLinearData(void *data, size_t dataLengthBytes) const
{
    ...
    if (data != NULL)
    {
        ...
        BitHelper::writeToBit(data, dataLengthBytes, &pos,
                              getRightParity(data, dataLengthBytes), 7, 1);
    }
}
...
void Getronik40BitFormat::setLinearData(const void *data, size_t dataLengthBytes)
{
    ...
    if (data != NULL)
    {
        ...
        BitHelper::extract(&fixedValue, 1, data, dataLengthBytes,
                           static_cast<unsigned int>(dataLengthBytes * 8), pos, 8);
        if (fixedValue != 0x2E)
        {
        ...
        fixedValue = 0x00;
        BitHelper::extract(&fixedValue, 1, data, dataLengthBytes,
                           static_cast<unsigned int>(dataLengthBytes * 8), pos, 1);
        ...
        d_formatLinear.d_field =
            (unsigned short)revertField(data, dataLengthBytes, &pos, 14);
        pos                  = 38;
