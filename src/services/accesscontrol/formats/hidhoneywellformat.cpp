...
size_t HIDHoneywellFormat::getFormatLinearData(void * /*data*/,
                                               size_t /*dataLengthBytes*/) const
{
    return 0;
}

void HIDHoneywellFormat::setFormatLinearData(const void * /*data*/,
                                             size_t * /*indexByte*/)
{
    // DOES NOTHING
}
...
unsigned char HIDHoneywellFormat::getRightParity(const void *data, size_t dataLengthBytes,
                                                 unsigned char rpNo) const
{
    ...
    if (data != NULL)
    {
        ...
        for (unsigned char i = 0; i < 8; ++i)
        {
            ...
            BitHelper::writeToBit(data, dataLengthBytes, &pos,
                                  getRightParity(data, dataLengthBytes, i), 7, 1);
        }
    }
}
...
void HIDHoneywellFormat::setLinearData(const void *data, size_t dataLengthBytes)
{
    ...
    if (data != NULL)
    {
        ...
        BitHelper::extract(&fixedValue, 1, data, dataLengthBytes,
                           static_cast<unsigned int>(dataLengthBytes * 8), pos, 4);
        pos += 4;
