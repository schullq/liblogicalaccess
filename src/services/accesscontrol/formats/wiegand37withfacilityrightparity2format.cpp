...
unsigned char
Wiegand37WithFacilityRightParity2Format::getLeftParity(const void *data,
                                                       size_t dataLengthBytes) const
{
    return calculateParity(data, dataLengthBytes, PT_ODD, 1, 16);
}

unsigned char
Wiegand37WithFacilityRightParity2Format::getRightParity1(const void *data,
                                                         size_t dataLengthBytes) const
{
...
unsigned char
Wiegand37WithFacilityRightParity2Format::getRightParity2(const void *data,
                                                         size_t dataLengthBytes) const
{
    return calculateParity(data, dataLengthBytes, PT_ODD, (size_t)0, 36);
}

void Wiegand37WithFacilityRightParity2Format::getLinearData(void *data,
                                                            size_t dataLengthBytes) const
{
    unsigned int pos = 1;
    ...
    if (data != NULL)
    {
        ...
        BitHelper::writeToBit(data, dataLengthBytes, &pos,
                              getLeftParity(data, dataLengthBytes), 7, 1);
        pos = 35;
        BitHelper::writeToBit(data, dataLengthBytes, &pos,
                              getRightParity1(data, dataLengthBytes), 7, 1);
        pos = 36;
        BitHelper::writeToBit(data, dataLengthBytes, &pos,
                              getRightParity2(data, dataLengthBytes), 7, 1);
    }
}

void Wiegand37WithFacilityRightParity2Format::setLinearData(const void *data,
                                                            size_t dataLengthBytes)
{
    ...
    if (data != NULL)
    {
        ...
        if ((unsigned char)((unsigned char)(reinterpret_cast<const unsigned char *>(
                                                data)[pos / 8]
                                            << (pos % 8)) >>
                            7) != parity)
        {
...
size_t
Wiegand37WithFacilityRightParity2Format::getFormatLinearData(void *data,
                                                             size_t dataLengthBytes) const
{
    ...
    {
        memcpy(&reinterpret_cast<unsigned char *>(data)[0], &d_formatLinear,
...
void Wiegand37WithFacilityRightParity2Format::setFormatLinearData(const void *data,
                                                                  size_t *indexByte)
{
