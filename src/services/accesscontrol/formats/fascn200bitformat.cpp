...
void FASCN200BitFormat::setLinearData(const void *data, size_t dataLengthBytes)
{
    ...
    setOrganizationalCategory(
        (FASCNOrganizationalCategory)revertField(data, dataLengthBytes, &pos, 4));
    setOrganizationalIdentifier(
        (unsigned char)revertField(data, dataLengthBytes, &pos, 16));
    setPOACategory(
        (FASCNPOAssociationCategory)revertField(data, dataLengthBytes, &pos, 4));
    ...
    {
        THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException,
                                 "The FASC-N LRC seems wrong.");
    }
}

unsigned char FASCN200BitFormat::calculateLRC(const void *data,
                                              unsigned int datalenBits) const
{
