    ...
    : Key()
{
    ...
    if (buf != NULL)
    {
        d_buf.insert(d_buf.end(), static_cast<const unsigned char *>(buf),
                     static_cast<const unsigned char *>(buf) + buflen);
        d_isEmpty = false;
    }
}
...
void BinaryDataField::getLinearData(void *data, size_t dataLengthBytes,
                                    unsigned int *pos) const
{
    ...
    size_t copyValueLength      = (d_value.getLength() > fieldDataLengthBytes)
                                 ? fieldDataLengthBytes
                                 : d_value.getLength();
    ...
    convertBinaryData(paddedBuffer, fieldDataLengthBytes, pos, d_length, data,
                      dataLengthBytes);
...
void BinaryDataField::setLinearData(const void *data, size_t dataLengthBytes,
                                    unsigned int *pos)
{
    if ((dataLengthBytes * 8) < (d_length + *pos))
    {
    ...
    revertBinaryData(data, dataLengthBytes, pos, d_length, paddedBuffer,
                     fieldDataLengthBytes);
