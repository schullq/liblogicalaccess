...
void ParityDataField::getLinearData(void *data, size_t dataLengthBytes,
                                    unsigned int *pos) const
{
    ...
    unsigned char parity = Format::calculateParity(data, dataLengthBytes, d_parityType,
                                                   positions, d_bitsUsePositions.size());
    delete[] positions;
    BitHelper::writeToBit(data, dataLengthBytes, pos, parity, 7, 1);
}

void ParityDataField::setLinearData(const void *data, size_t dataLengthBytes,
                                    unsigned int *pos)
{
    if ((dataLengthBytes * 8) < (d_length + *pos))
    {
