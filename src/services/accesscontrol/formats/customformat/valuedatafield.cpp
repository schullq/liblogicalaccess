...
void ValueDataField::convertNumericData(void *data, size_t dataLengthBytes,
                                        unsigned int *pos, unsigned long long field,
                                        unsigned int fieldlen) const
{
    unsigned int convertedDataTypeLengthBits =
        d_dataType->convert(field, fieldlen, NULL, 0);
    if (convertedDataTypeLengthBits > 0)
    {
        size_t convertedDataTypeLengthBytes = (convertedDataTypeLengthBits + 7) / 8;
        unsigned char *convertedDataTypeData =
            new unsigned char[convertedDataTypeLengthBytes];
        memset(convertedDataTypeData, 0x00, convertedDataTypeLengthBytes);

        if (d_dataType->convert(field, fieldlen, convertedDataTypeData,
                                convertedDataTypeLengthBytes) ==
            convertedDataTypeLengthBits)
        {
            unsigned int convertedDataRepresentationLengthBits =
                d_dataRepresentation->convertNumeric(
                    convertedDataTypeData, convertedDataTypeLengthBytes,
                    convertedDataTypeLengthBits, NULL, 0);
            if (convertedDataRepresentationLengthBits > 0)
            {
                size_t convertedDataRepresentationLengthBytes =
                    (convertedDataRepresentationLengthBits + 7) / 8;
                unsigned char *convertedDataRepresentationData =
                    new unsigned char[convertedDataRepresentationLengthBytes];

                if (d_dataRepresentation->convertNumeric(
                        convertedDataTypeData, convertedDataTypeLengthBytes,
                        convertedDataTypeLengthBits, convertedDataRepresentationData,
                        convertedDataRepresentationLengthBytes) ==
                    convertedDataRepresentationLengthBits)
                {
                    BitHelper::writeToBit(data, dataLengthBytes, pos,
                                          convertedDataRepresentationData,
                                          convertedDataRepresentationLengthBytes,
                                          convertedDataRepresentationLengthBits, 0,
                                          convertedDataRepresentationLengthBits);
                }
...
void ValueDataField::convertBinaryData(const void *data, size_t dataLengthBytes,
                                       unsigned int *pos, unsigned int fieldlen,
                                       void *convertedData,
                                       size_t convertedDataLengthBytes) const
{
    unsigned int convertedDataRepresentationLengthBits =
        d_dataRepresentation->convertBinary(data, dataLengthBytes, fieldlen, NULL, 0);
    if (convertedDataRepresentationLengthBits > 0)
    {
        size_t convertedDataRepresentationLengthBytes =
            (convertedDataRepresentationLengthBits + 7) / 8;
        ...
        if (d_dataRepresentation->convertBinary(data, dataLengthBytes, fieldlen,
                                                convertedDataRepresentationData,
                                                convertedDataRepresentationLengthBytes) ==
            convertedDataRepresentationLengthBits)
        {
            BitHelper::writeToBit(convertedData, convertedDataLengthBytes, pos,
                                  convertedDataRepresentationData,
                                  convertedDataRepresentationLengthBytes,
                                  convertedDataRepresentationLengthBits, 0,
                                  convertedDataRepresentationLengthBits);
...
unsigned long long ValueDataField::revertNumericData(const void *data,
                                                     ...
                                                     unsigned int fieldlen) const
{
    ...
    unsigned int extractedSizeBits =
        d_dataType->convert(0, d_dataRepresentation->convertLength(fieldlen), NULL, 0);
    ...
    unsigned int tmp = 0;
    if ((tmp = BitHelper::extract(extractData, extractedSizeBytes, data, dataLengthBytes,
                                  static_cast<unsigned int>(dataLengthBytes * 8), *pos,
                                  extractedSizeBits)) > 0)
    {
        if ((tmp = d_dataRepresentation->revertNumeric(extractData, extractedSizeBytes,
                                                       tmp, revertedData,
                                                       revertedSizeBytes)) > 0)
        {
...
void ValueDataField::revertBinaryData(const void *data, size_t dataLengthBytes,
                                      unsigned int *pos, unsigned int fieldlen,
                                      void *revertedData,
                                      size_t revertedDataLengthBytes) const
{
    ...
    if ((tmp = BitHelper::extract(extractData, extractedSizeBytes, data, dataLengthBytes,
                                  ...
                                  readSizeBits)) > 0)
    {
        if ((tmp = d_dataRepresentation->revertBinary(extractData, extractedSizeBytes,
                                                      tmp, revertedTemporaryData,
                                                      revertedTemporarySizeBytes)) > 0)
        {
            ...
            if (revertedDataLengthBytes < revertedTemporarySizeBytes)
                THROW_EXCEPTION_WITH_LOG(
                    ...
                    "The size of the dest buffer is too small for memcpy");
            memcpy(revertedData, revertedTemporaryData, revertedTemporarySizeBytes);
#else
            memcpy_s(revertedData, revertedDataLengthBytes, revertedTemporaryData,
                     revertedTemporarySizeBytes);
#endif
        }
    }
