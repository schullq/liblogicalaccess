...
unsigned char Corporate1000Format::getLeftParity1(const void *data,
...
unsigned char Corporate1000Format::getLeftParity2(const void *data,
                                                  size_t dataLengthBytes) const
{
    unsigned char parity = 0x00;
    ...
    if (data != NULL)
    {
        ...
        positions[20] = 32;
        positions[21] = 33;
        parity        = Format::calculateParity(data, dataLengthBytes, PT_EVEN, positions,
                                         sizeof(positions) / sizeof(int));
    }

    return parity;
}

unsigned char Corporate1000Format::getRightParity(const void *data,
                                                  size_t dataLengthBytes) const
{
    unsigned char parity = 0x00;
    ...
    if (data != NULL)
    {
        ...
        positions[20] = 31;
        positions[21] = 32;
        parity        = Format::calculateParity(data, dataLengthBytes, PT_ODD, positions,
                                         sizeof(positions) / sizeof(int));
    }
...
void Corporate1000Format::getLinearData(void *data, size_t dataLengthBytes) const
{
    ...
    if (data != NULL)
    {
        ...
        BitHelper::writeToBit(data, dataLengthBytes, &pos,
                              getLeftParity2(data, dataLengthBytes), 7, 1);
        pos = 34;
        BitHelper::writeToBit(data, dataLengthBytes, &pos,
                              getRightParity(data, dataLengthBytes), 7, 1);
        pos = 0;
        BitHelper::writeToBit(data, dataLengthBytes, &pos,
                              getLeftParity1(data, dataLengthBytes), 7, 1);
    }
}
