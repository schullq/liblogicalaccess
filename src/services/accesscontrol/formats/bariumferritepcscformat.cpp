...
size_t BariumFerritePCSCFormat::getFormatLinearData(void * /*data*/,
                                                    size_t /*dataLengthBytes*/) const
{
    return 0;
}

void BariumFerritePCSCFormat::setFormatLinearData(const void * /*data*/,
                                                  size_t * /*indexByte*/)
{
    // DOES NOTHING
}
...
unsigned char BariumFerritePCSCFormat::calcChecksum(const unsigned char *data,
...
void BariumFerritePCSCFormat::getLinearData(void *data, size_t dataLengthBytes) const
{
    ...
    if (data != NULL)
    {
        ...
        BitHelper::writeToBit(data, dataLengthBytes, &pos,
                              calcChecksum(reinterpret_cast<const unsigned char *>(data),
                                           static_cast<unsigned int>((pos + 7) / 8)));
    }
}
...
void BariumFerritePCSCFormat::setLinearData(const void *data, size_t dataLengthBytes)
{
    if (data != NULL)
    {
        ...
        unsigned char checksum =
            calcChecksum(reinterpret_cast<const unsigned char *>(data), checkOffset);
        unsigned char currentChecksum =
            reinterpret_cast<const unsigned char *>(data)[checkOffset];
