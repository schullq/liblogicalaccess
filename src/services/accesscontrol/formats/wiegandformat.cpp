...
unsigned char WiegandFormat::getLeftParity() const
{
    ...
    unsigned char ret =
        calculateParity(data, dataLength, d_leftParityType, 1, d_leftParityLength);
...
unsigned char WiegandFormat::getRightParity() const
{
    ...
    getLinearDataWithoutParity(data, dataLength);
    unsigned char ret =
        calculateParity(data, dataLength, d_rightParityType,
...
void WiegandFormat::setLinearData(const void *data, size_t dataLengthBytes)
{
    ...
    {
        par = getLeftParity();
        if ((unsigned char)(reinterpret_cast<const unsigned char *>(data)[pos / 8] >>
                            7) != par)
        {
