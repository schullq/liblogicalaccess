...
unsigned int BCDNibbleDataType::convert(unsigned long long data,
                                        unsigned int dataLengthBits, void *dataConverted,
                                        size_t dataConvertedLengthBytes)
{
    unsigned int ret = 0;
    ...
    ret = DataType::addParityToBuffer(d_leftParityType, d_rightParityType, 4, NULL, shft,
                                      NULL, 0);
    ...
    if (dataConverted != NULL)
    {
        if (dataConvertedLengthBytes >= i)
        {
            ...
            DataType::addParityToBuffer(
                d_leftParityType, d_rightParityType, 4, tmpswb2, shft, swb,
...
unsigned long long BCDNibbleDataType::revert(void *data, size_t dataLengthBytes,
                                             unsigned int lengthBits)
{
    ...
    if (data != NULL && dataLengthBytes > 0)
    {
        unsigned int tmpswblen = DataType::removeParityToBuffer(
            d_leftParityType, d_rightParityType, 4, NULL, lengthBits, NULL, 0);
        ...
        DataType::removeParityToBuffer(d_leftParityType, d_rightParityType, 4, data,
                                       lengthBits, tmpswb,
                                       static_cast<unsigned int>(tmpswblenBytes * 8));

        size_t i;
        int coef;

        for (i = 0, coef = static_cast<int>(((tmpswblen + 3) / 4) - 1);
             i < tmpswblenBytes; ++i, coef -= 2)
        {
            if (d_bitDataRepresentationType == ET_LITTLEENDIAN)
            {
                tmpswb[i] = ((0x0F & DataType::invertBitSex(tmpswb[i] >> 4, 4)) << 4) |
                            (tmpswb[i] & 0x0F);
                tmpswb[i] =
                    (0x0F & DataType::invertBitSex(tmpswb[i], 4)) | (tmpswb[i] & 0xF0);
            }
