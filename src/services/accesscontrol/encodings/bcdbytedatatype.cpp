...
unsigned int BCDByteDataType::convert(unsigned long long data,
                                      ...
                                      size_t dataConvertedLengthBytes)
{
    ...
    ret = DataType::addParityToBuffer(d_leftParityType, d_rightParityType, 8, NULL, i * 8,
                                      NULL, 0);
    ...
    if (dataConverted != NULL)
    {
        if (dataConvertedLengthBytes >= i)
        {
            ...
            DataType::addParityToBuffer(
                d_leftParityType, d_rightParityType, 8, tmpswb, i * 8, swb,
...
unsigned long long BCDByteDataType::revert(void *data, size_t dataLengthBytes,
                                           unsigned int lengthBits)
{
    ...
    {
        size_t tmpswblen = DataType::removeParityToBuffer(
        ...
        for (i = 0, coef = static_cast<int>(dataLengthBytes - 1); i < dataLengthBytes;
             ++i, --coef)
        {
            if (d_bitDataRepresentationType == ET_LITTLEENDIAN)
            {
