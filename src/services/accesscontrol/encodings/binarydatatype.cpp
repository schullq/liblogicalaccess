...
unsigned int BinaryDataType::convert(unsigned long long data, unsigned int dataLengthBits,
                                     void *dataConverted, size_t dataConvertedLengthBytes)
{
    unsigned int ret = 0;

    /*if (dataLengthBits <= 8)
    {
    ret = DataType::addParityToBuffer(d_leftParityType, d_rightParityType, 8, NULL,
    dataLengthBits, NULL, 0);
    ...
    DataType::addParityToBuffer(d_leftParityType, d_rightParityType, 8, tmpswb,
    dataLengthBits, dataConverted, dataConvertedLengthBytes * 8);
    delete[] tmpswb;
    }
    }
    else
    {*/
    unsigned int tmpswblen = DataType::addParityToBuffer(
        d_leftParityType, d_rightParityType, 8, NULL,
        static_cast<unsigned int>(dataConvertedLengthBytes * 8), NULL, 0);
    ...
    DataType::addParityToBuffer(d_leftParityType, d_rightParityType, 8, tmpswb, tmpswblen,
                                dataConverted,
                                static_cast<unsigned int>(dataConvertedLengthBytes * 8));
    delete[] tmpswb;
    //}
...
unsigned long long BinaryDataType::revert(void *data, size_t dataLengthBytes,
                                          unsigned int lengthBits)
{
    ...
    {
        unsigned int tmpswblen = DataType::removeParityToBuffer(
            d_leftParityType, d_rightParityType, 8, NULL, lengthBits, NULL, 0);
        size_t tmpswblenBytes = (tmpswblen + 7) / 8;
        unsigned char *tmpswb = new unsigned char[tmpswblenBytes];
        memset(tmpswb, 0x00, tmpswblenBytes);

        DataType::removeParityToBuffer(d_leftParityType, d_rightParityType, 8, data,
                                       lengthBits, tmpswb, tmpswblen);
        ...
        for (long long n = (static_cast<long long>(dataLengthBytes - 1) * 8); n >= 0;
