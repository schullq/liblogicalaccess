...
unsigned int DataType::addParityToBuffer(ParityType leftParity, ParityType rightParity,
                                         unsigned int blocklen, void *buf,
                                         unsigned int buflen, void *procbuf,
                                         unsigned int procbuflen)
{
    ...
    unsigned int newsize  = buflen + (nbblocks * (((leftParity != PT_NONE) ? 1 : 0) +
                                                 ((rightParity != PT_NONE) ? 1 : 0)));
        ...
        while (pos < buflen)
        {
            if (leftParity != PT_NONE)
            {
                unsigned char parity =
                    StaticFormat::calculateParity(buf, buflen, leftParity, pos, blocklen);
                BitHelper::writeToBit(procbuf, procbuflenBytes, &posproc, parity, 7, 1);
            }

            unsigned int currentBlocklen =
                (pos + blocklen <= buflen) ? blocklen : buflen - pos;
            BitHelper::writeToBit(procbuf, procbuflenBytes, &posproc, buf, buflenBytes,
                                  buflen, pos, currentBlocklen);

            if (rightParity != PT_NONE)
            {
                unsigned char parity = StaticFormat::calculateParity(
                    buf, buflen, rightParity, pos, blocklen);
                BitHelper::writeToBit(procbuf, procbuflenBytes, &posproc, parity, 7, 1);
            }
...
unsigned int DataType::removeParityToBuffer(ParityType leftParity, ParityType rightParity,
                                            unsigned int blocklen, void *buf,
                                            unsigned int buflen, void *procbuf,
                                            unsigned int procbuflen)
{
    unsigned int coefParity =
        (((leftParity != PT_NONE) ? 1 : 0) + ((rightParity != PT_NONE) ? 1 : 0));
    ...
    {
        char tmpmsg[256];
        sprintf(tmpmsg, "The buffer length (%d) without parity is not block length "
                        "aligned. If you use parity on data type, it must be aligned.",
        ...
        if (buflen > procbuflen)
        {
            ...
            while (pos < buflen)
            {
                if (leftParity != PT_NONE)
                {
                    ...
                    BitHelper::writeToBit(&parityToCheck, 1, &posparity, buf, buflenBytes,
                                          buflen, pos, 1);
                    unsigned char parity = StaticFormat::calculateParity(
                        buf, buflen, leftParity, pos++, blocklen);
                ...
                unsigned int currentBlocklen =
                    ((pos + blocklen + ((rightParity != PT_NONE) ? 1 : 0)) <= buflen)
                        ? blocklen
                        : buflen - pos - ((rightParity != PT_NONE) ? 1 : 0);
                BitHelper::writeToBit(procbuf, procbuflenBytes, &posproc, buf,
                                      buflenBytes, buflen, pos, currentBlocklen);
                pos += currentBlocklen;
                ...
                if (rightParity != PT_NONE)
                {
                    ...
                    BitHelper::writeToBit(&parityToCheck, 1, &posparity, buf, buflenBytes,
                                          buflen, pos, 1);
                    unsigned char parity = StaticFormat::calculateParity(
                        buf, buflen, rightParity, pos++ - blocklen, blocklen);
