...
NdefMessage::NdefMessage(const std::vector<unsigned char> &data)
{
    ...
    while (index < data.size())
    {
        ...
        record->setType(std::vector<unsigned char>(data.begin() + index,
                                                   data.begin() + index + typeLength));
        index += typeLength;
        ...
        if (il)
        {
            ...
            record->setId(std::vector<unsigned char>(data.begin() + index,
                                                     data.begin() + index + idLength));
            index += idLength;
        }

        EXCEPTION_ASSERT((index + payloadLength) <= data.size(), std::invalid_argument,
                         "The buffer size is too small (5).");
        record->setPayload(std::vector<unsigned char>(
            data.begin() + index, data.begin() + index + payloadLength));
        index += payloadLength;
...
std::vector<unsigned char> NdefMessage::encode()
{
         ...
         it != m_records.end(); ++it)
    {
        std::vector<unsigned char> record =
            (*it)->encode((it == m_records.begin()), (std::next(it) == m_records.end()));
...
std::shared_ptr<NdefMessage> NdefMessage::TLVToNdefMessage(std::vector<unsigned char> tlv)
{
    ...
    while (i + 1u < tlv.size())
    {
        ...
        case 0x03: // Ndef message
            ...
            {
                std::vector<unsigned char> msgdata(tlv.begin() + i + 1,
                                                   tlv.begin() + i + 1 + tlv[i]);
...
NdefMessage::NdefMessageToTLV(std::shared_ptr<NdefMessage> message)
{
    std::vector<unsigned char> data;
