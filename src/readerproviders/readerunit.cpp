...
bool ReaderUnit::waitInsertion(const std::vector<unsigned char> &identifier,
...
ReaderUnit::getNumber(std::shared_ptr<Chip> chip,
                      std::shared_ptr<CardsFormatComposite> composite)
{
    ...
    if (composite)
    {
        ...
        if (ctList.size() > 0)
        {
            ...
            CardTypeList::iterator itct =
                std::find(ctList.begin(), ctList.end(), useCardType);
...
uint64_t ReaderUnit::getFormatedNumber(const std::vector<unsigned char> &number,
                                       int padding)
{
