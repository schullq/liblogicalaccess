    ...
    : d_level(level)
{
    ...
    if (logfile && d_level != NONE)
    {
        ...
        {
            _stream << Colorize::underline(boost::posix_time::to_simple_string(now))
                    << " - " << Colorize::red(logLevelMsg[d_level]) << ": \t{" << line
                    << "}\t{" << Colorize::green(func) << "}\t{" << file
                    << "}:" << std::endl;
        }
        else
        {
...
std::ostream &operator<<(std::ostream &ss, const std::vector<unsigned char> &bytebuff)
{
