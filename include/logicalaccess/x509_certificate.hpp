 ...
 *
 * X509Certificate represents a X509 certificate. X509Certificate can be loaded from a
 * file (createFromFile()), from memory (createFromPEM()) or created from scratch
 * (createSelfSignedFromScratch()).
 *
 * You can also get a X509Certificate from a X509CertificateRequest (see
 * createCASignedFromX509CertificateRequest()) or get a X509CertificateRequest from a
 * X509Certificate (see toSignedX509CertificateRequest()).
 *
 * X509Certificate is nullable. Before using one of its method you must ensure that the
...
class X509Certificate
{
  public:
     ...
     */
    static X509Certificate createFromFile(const std::string &filename,
                                          PEMPassphraseCallback callback = NULL,
                                          void *userdata                 = NULL);
     ...
     * \return The X509Certificate.
     */
    static X509Certificate createFromPEM(const std::vector<unsigned char> &data,
                                         PEMPassphraseCallback callback = NULL,
                                         void *userdata                 = NULL);

    /**
     * \brief Create a self-signed X509 certificate.
     * \param pkey The private key to use. If pkey is not a RSAKey or if it has no private
     * compound, the behavior is undefined.
    ...
    createSelfSignedFromScratch(const EVPPKey &pkey, const X509Name &name,
                                const boost::posix_time::ptime &notBefore,
                                const boost::posix_time::ptime &notAfter,
     ...
     * \param ca_pkey The CA private key. If ca_pkey is null or has no private compound,
     ...
     */
    static X509Certificate
    createCASignedFromX509CertificateRequest(const X509CertificateRequest &req,
                                             const X509Certificate &ca_cert,
                                             const EVPPKey &ca_pkey, int duration = 365,
                                             int digest_nid = NID_sha1WithRSAEncryption);
     ...
     * \param pkey The public key. If key is NULL or has a private compound, the behavior
     * is undefined.
     ...
     * \param pkey The private key. If key is NULL or has no private compound, the
    ...
    toSignedX509CertificateRequest(const EVPPKey &pkey,
                                   int digest_nid = NID_sha1WithRSAEncryption) const;
  ...
  private:
     ...
     */
    static void addExtension(std::shared_ptr<X509> cert, std::shared_ptr<X509V3_CTX> ctx,
     ...
     * \param digest_nid The digest nid. Default is NID_sha1WithRSAEncryption.
     */
    static void signCertificateWithKey(std::shared_ptr<X509> cert, const EVPPKey &pkey,
                                       int digest_nid = NID_sha1WithRSAEncryption);

    /**
     * \brief Check if a specified NID must can be copied from CA certificate to a client
     * certificate.
     * \param nid The NID.
