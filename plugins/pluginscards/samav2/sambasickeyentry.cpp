...
std::vector<std::vector<unsigned char>> SAMBasicKeyEntry::getKeysData()
{
    ...
    for (unsigned char x = 0; x < keynb; ++x)
        ret.push_back(std::vector<unsigned char>(d_key + (x * keysize),
                                                 d_key + (x * keysize) + keysize));
    return ret;
}

void SAMBasicKeyEntry::setKeysData(std::vector<std::vector<unsigned char>> keys,
                                   SAMKeyType type)
{
    if (keys.size() == 0)
        return;
