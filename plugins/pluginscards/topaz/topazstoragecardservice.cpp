...
TopazStorageCardService::readData(std::shared_ptr<Location> location,
                                  ...
                                  CardBehavior behaviorFlags)
{
    ...
    if (nbPages >= 1)
    {
        ...
        ret.insert(ret.end(), dataPages.begin() + tLocation->byte,
                   dataPages.begin() + tLocation->byte + length);
    }
    return ret;
}
