...
class LIBLOGICALACCESS_API MifarePlusSL1Commands : public MifareCommands
{
  ...
  private:
    bool aes_auth_step2(std::vector<uint8_t> rnd_b, std::shared_ptr<AES128Key> key);
...
*/
template <typename MifareSL1Impl, typename MifareClassicImpl>
class MifarePlusSL1Policy : public MifarePlusSL1Commands
{
  public:
    ...
    virtual std::vector<unsigned char> readBinary(unsigned char blockno,
                                                  size_t len) override
    {
        fixup();
    ...
    virtual bool loadKey(unsigned char keyno, MifareKeyType keytype,
                         std::shared_ptr<MifareKey> key, bool vol) override
    {
        fixup();
