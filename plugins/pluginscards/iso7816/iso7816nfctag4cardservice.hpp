...
class LIBLOGICALACCESS_API ISO7816NFCTag4CardService : public NFCTagCardService
{
  public:
    ...
    virtual void writeNDEFFile(std::shared_ptr<logicalaccess::NdefMessage> records,
                               unsigned short isoFIDNDEFFile = 0xe104);

    virtual void writeNDEFFile(std::vector<unsigned char> recordsData,
                               unsigned short isoFIDNDEFFile = 0xe104);

    virtual std::shared_ptr<logicalaccess::NdefMessage>
  ...
  protected:
    std::shared_ptr<ISO7816Chip> getISO7816Chip()
    {
