...
bool DESFireCrypto::verifyMAC(bool end, const std::vector<unsigned char> &data)
{
    ...
    if (end)
    {
        ...
        else
        {
            ...
            std::vector<unsigned char> ourMac =
...
DESFireCrypto::generateMAC(const std::vector<unsigned char> &data)
{
    std::vector<unsigned char> ret;
...
std::vector<unsigned char>
DESFireCrypto::desfireEncrypt(const std::vector<unsigned char> &data,
...
std::vector<unsigned char>
DESFireCrypto::encipherData(bool end, std::vector<unsigned char> data,
                            const std::vector<unsigned char> &param)
{
    if (d_auth_method != CM_LEGACY)
    {
...
std::vector<unsigned char>
DESFireCrypto::desfire_CBC_send(const std::vector<unsigned char> &key,
...
std::vector<unsigned char>
DESFireCrypto::desfire_CBC_receive(const std::vector<unsigned char> &key,
                                   const std::vector<unsigned char> &iv,
                                   const std::vector<unsigned char> &data)
{
    symmetric_key skey;
...
std::vector<unsigned char>
DESFireCrypto::desfire_CBC_mac(const std::vector<unsigned char> &key,
                               const std::vector<unsigned char> &iv,
                               const std::vector<unsigned char> &data)
{
...
std::vector<unsigned char>
DESFireCrypto::sam_CBC_send(const std::vector<unsigned char> &key,
                            const std::vector<unsigned char> &iv,
                            const std::vector<unsigned char> &data)
{
...
std::vector<unsigned char>
DESFireCrypto::desfire_mac(const std::vector<unsigned char> &key,
...
std::vector<unsigned char>
DESFireCrypto::desfire_encrypt(const std::vector<unsigned char> &key,
                               std::vector<unsigned char> data)
{
...
std::vector<unsigned char>
DESFireCrypto::sam_encrypt(const std::vector<unsigned char> &key,
                           std::vector<unsigned char> data)
{
    int pad = (8 - ((data.size() + 2) % 8)) % 8;
...
std::vector<unsigned char>
DESFireCrypto::desfire_decrypt(const std::vector<unsigned char> &key,
                               const std::vector<unsigned char> &data, size_t datalen)
{
    std::vector<unsigned char> ret;
    size_t ll = 0;
...
std::vector<unsigned char>
DESFireCrypto::authenticate_PICC1(unsigned char keyno,
                                  std::vector<unsigned char> diversify,
                                  const std::vector<unsigned char> &encRndB)
{
    d_sessionKey.clear();
    d_authkey.resize(16);
...
void DESFireCrypto::authenticate_PICC2(unsigned char keyno,
                                       const std::vector<unsigned char> &encRndA1)
{
    std::vector<unsigned char> rndA =
        desfire_CBC_send(d_authkey, std::vector<unsigned char>(), encRndA1);
...
void DESFireCrypto::getKey(std::shared_ptr<DESFireKey> key,
                           std::vector<unsigned char> diversify,
...
void DESFireCrypto::getKeyVersioned(std::shared_ptr<DESFireKey> key,
                                    std::vector<unsigned char> &keyversioned)
{
    ...
    keyversioned.clear();
    keyversioned.insert(keyversioned.end(), keytmpversioned,
                        keytmpversioned + key->getLength());
...
std::vector<unsigned char>
DESFireCrypto::changeKey_PICC(unsigned char keyno, std::shared_ptr<DESFireKey> newkey,
                              std::vector<unsigned char> diversify)
{
    ...
    if (d_auth_method == CM_LEGACY) // Native DESFire
    {
        if (keyno_only != d_currentKeyNo)
        {
            ...
            cryptogram = desfire_CBC_send(d_sessionKey, std::vector<unsigned char>(),
                                          encCryptogram);
        }
        else
        {
    ...
    else
    {
        if (keyno_only != d_currentKeyNo)
        {
            ...
            d_lastIV = std::vector<unsigned char>(cryptogram.end() - d_block_size,
                                                  cryptogram.end());
        }
        else
        {
            ...
            cryptogram = desfire_iso_encrypt(d_sessionKey, newkeydiv, d_cipher,
                                             d_block_size, encCryptogram);
        }
    }
...
std::vector<unsigned char> DESFireCrypto::iso_authenticate_PICC1(
    unsigned char keyno, std::vector<unsigned char> diversify,
    const std::vector<unsigned char> &encRndB, unsigned int randomlen)
{
    d_sessionKey.clear();
    getKey(d_currentAid, keyno, diversify, d_authkey);
...
std::vector<unsigned char>
DESFireCrypto::aes_authenticate_PICC1(unsigned char keyno,
                                      std::vector<unsigned char> diversify,
                                      const std::vector<unsigned char> &encRndB)
{
    d_sessionKey.clear();
...
std::vector<unsigned char>
DESFireCrypto::desfire_cmac(const std::vector<unsigned char> &data)
{
    return desfire_cmac(d_sessionKey, d_cipher, d_block_size, data);
}

std::vector<unsigned char>
DESFireCrypto::desfire_cmac(const std::vector<unsigned char> &key,
...
std::vector<unsigned char>
DESFireCrypto::desfire_iso_decrypt(const std::vector<unsigned char> &data, size_t length)
{
    return desfire_iso_decrypt(d_sessionKey, data, d_cipher, d_block_size, length);
}

std::vector<unsigned char> DESFireCrypto::desfire_iso_decrypt(
    const std::vector<unsigned char> &key, const std::vector<unsigned char> &data,
    ...
    size_t datalen)
{
    ...
    std::vector<unsigned char> crcbuf =
        std::vector<unsigned char>(decdata.begin(), decdata.begin() + ll);
    ...
    std::vector<unsigned char> padding = std::vector<unsigned char>(
        decdata.begin() + ll + 4, decdata.begin() + ll + 4 + pad);
...
std::vector<unsigned char>
DESFireCrypto::iso_encipherData(bool end, const std::vector<unsigned char> &data,
                                const std::vector<unsigned char> &param)
{
    ...
    if (decdata.size() > 0)
    {
        ...
        d_lastIV =
            std::vector<unsigned char>(encdata.end() - d_block_size, encdata.end());
    }

    return encdata;
}

std::vector<unsigned char> DESFireCrypto::desfire_iso_encrypt(
    const std::vector<unsigned char> &key, const std::vector<unsigned char> &data,
    std::shared_ptr<openssl::OpenSSLSymmetricCipher> cipher, unsigned int block_size,
    const std::vector<unsigned char> &param)
{
...
void DESFireCrypto::createApplication(int aid, size_t maxNbKeys,
...
bool DESFireCrypto::getKey(unsigned char keyno, std::vector<unsigned char> diversify,
                           std::vector<unsigned char> &keydiv)
{
    return getKey(d_currentAid, keyno, diversify, keydiv);
}

bool DESFireCrypto::getKey(size_t aid, unsigned char keyno,
                           std::vector<unsigned char> diversify,
                           std::vector<unsigned char> &keydiv)
{
    if (!checkKeyPos(aid, keyno, false))
...
void DESFireCrypto::setKey(size_t aid, unsigned char keyno,
                           std::shared_ptr<DESFireKey> key)
{
