...
{
void NXPAV2KeyDiversification::initDiversification(std::vector<unsigned char> identifier,
                                                   int AID, std::shared_ptr<Key> key,
                                                   unsigned char keyno,
...
NXPAV2KeyDiversification::getDiversifiedKey(std::shared_ptr<Key> key,
                                            std::vector<unsigned char> diversify)
{
    ...
    if (std::dynamic_pointer_cast<DESFireKey>(key)->getKeyType() ==
            DESFireKeyType::DF_KEY_DES ||
        std::dynamic_pointer_cast<DESFireKey>(key)->getKeyType() ==
            DESFireKeyType::DF_KEY_3K3DES)
    {
        d_cipher.reset(new openssl::DESCipher());
        block_size = 8;
    }
    else if (std::dynamic_pointer_cast<DESFireKey>(key)->getKeyType() ==
        ...
        DESFireKeyType::DF_KEY_AES)
    {
        ...
        keydiv_tmp = openssl::CMACCrypto::cmac(keycipher, d_cipher, block_size, diversify,
                                               emptyIV, 32);
        keydiv.resize(16);
             ...
             DESFireKeyType::DF_KEY_DES)
    {
        ...
        keydiv_tmp_1 = openssl::CMACCrypto::cmac(keycipher, d_cipher, block_size,
                                                 diversify, emptyIV, 16);
        diversify[0] = 0x22;
        keydiv_tmp_2 = openssl::CMACCrypto::cmac(keycipher, d_cipher, block_size,
                                                 diversify, emptyIV, 16);
             ...
             DESFireKeyType::DF_KEY_3K3DES)
    {
        ...
        keydiv_tmp_2 = openssl::CMACCrypto::cmac(keycipher, d_cipher, block_size,
                                                 diversify, emptyIV, 16);
        diversify[0] = 0x33;
        keydiv_tmp_3 = openssl::CMACCrypto::cmac(keycipher, d_cipher, block_size,
                                                 diversify, emptyIV, 16);
        keydiv.insert(keydiv.end(), keydiv_tmp_1.begin() + 8, keydiv_tmp_1.end());
        keydiv.insert(keydiv.end(), keydiv_tmp_2.begin() + 8, keydiv_tmp_2.end());
