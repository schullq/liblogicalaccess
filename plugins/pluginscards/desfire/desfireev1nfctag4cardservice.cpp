...
void DESFireEV1NFCTag4CardService::createNFCApplication(
    unsigned int aid, std::shared_ptr<logicalaccess::DESFireKey> masterPICCKey,
...
{
    std::shared_ptr<logicalaccess::DESFireCommands> desfirecommand(
        std::dynamic_pointer_cast<logicalaccess::DESFireCommands>(
            getChip()->getCommands()));
    std::shared_ptr<logicalaccess::DESFireEV1Commands> desfireev1command(
        std::dynamic_pointer_cast<logicalaccess::DESFireEV1Commands>(
            getChip()->getCommands()));
    ...
    desfireev1command->createApplication(
        aid, logicalaccess::DESFireKeySettings::KS_DEFAULT, 1,
        logicalaccess::DESFireKeyType::DF_KEY_DES,
        logicalaccess::FidSupport::FIDS_ISO_FID, isoFIDApplication, dfNameVector);

    std::shared_ptr<logicalaccess::DESFireKey> key(
        new logicalaccess::DESFireKey("00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00"));
    ...
    desfireev1command->createStdDataFile(0x01, logicalaccess::EncryptionMode::CM_PLAIN,
                                         dar, 0x0f, isoFIDCapabilityContainer);
    ...
    desfireev1command->createStdDataFile(0x02, logicalaccess::EncryptionMode::CM_PLAIN,
                                         dar, 0xff, isoFIDNDEFFile);
}
...
std::shared_ptr<logicalaccess::NdefMessage>
...
void DESFireEV1NFCTag4CardService::writeNDEF(
    std::shared_ptr<logicalaccess::NdefMessage> records)
{
    std::shared_ptr<logicalaccess::DESFireKey> picckey(
        new logicalaccess::DESFireKey("00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00"));
