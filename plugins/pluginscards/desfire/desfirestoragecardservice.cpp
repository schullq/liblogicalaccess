...
void DESFireStorageCardService::erase()
{
    ...
    cmd->changeKey(0, std::shared_ptr<DESFireKey>(new DESFireKey(std::string(
                                      ...
                                      std::shared_ptr<AccessInfo> aiToUse)
{
    ...
    else if (!dfAiToUse->masterApplicationKey->isEmpty())
    {
        ...
        std::vector<unsigned char> files =
            getDESFireChip()->getDESFireCommands()->getFileIDs();
        for (std::vector<unsigned char>::const_iterator file = files.cbegin();
        ...
        getDESFireChip()->getDESFireCommands()->changeKey(
            0, std::shared_ptr<DESFireKey>(new DESFireKey(
                                          ...
                                          CardBehavior /*behaviorFlags*/)
{
    ...
    else
    {
        getDESFireChip()->getCrypto()->setKey(
            0, 0, std::shared_ptr<DESFireKey>(new DESFireKey(
                      std::string("00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00"))));
    }
    ...
    if (!dfAiToUse->writeKey->isEmpty() && dfAiToUse->writeKeyno != 0x00)
    {
        ...
        getDESFireChip()->getCrypto()->setKey(dfLocation->aid, dfAiToUse->writeKeyno,
                                              dfAiToUse->writeKey);
    }
    ...
    if (!dfAiToUse->readKey->isEmpty() && dfAiToUse->readKeyno != 0x00)
    {
        ...
        getDESFireChip()->getCrypto()->setKey(dfLocation->aid, dfAiToUse->readKeyno,
                                              dfAiToUse->readKey);
    }
    ...
    try
    {
        if (createArbo && dfAiToWrite)
        {
            ...
            getDESFireChip()->getDESFireCommands()->createStdDataFile(
                dfLocation, rights, (int)(data.size() + dfLocation->byte));
            needLoadKey = false;
        }
    }
    catch (std::exception &)
    ...
    getDESFireChip()->getDESFireCommands()->writeData(dfLocation->file, dfLocation->byte,
                                                      data, encMode);
                ...
                dfAiToUse->masterCardKey != dfAiToWrite->masterCardKey)
            {
                try
                {
                    ...
                    LOG(LogLevel::INFOS)
                        ...
                        << (dfAiToWrite->masterCardKey->getKeyDiversification() == NULL);
                    getDESFireChip()->getDESFireCommands()->changeKey(
...
DESFireStorageCardService::readData(std::shared_ptr<Location> location,
                                    std::shared_ptr<AccessInfo> aiToUse,
                                    size_t dataLength, CardBehavior /*behaviorFlags*/)
{
    EXCEPTION_ASSERT_WITH_LOG(location, std::invalid_argument,
                              "location cannot be null.");
    ...
    if (needLoadKey)
    {
        ...
        else if (dfAiToUse->readKeyno == TaskAccessRights::AR_KEY0 &&
                 dfAiToUse->masterApplicationKey &&
    ...
    return getDESFireChip()->getDESFireCommands()->readData(
