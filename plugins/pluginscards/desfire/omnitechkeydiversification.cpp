...
{
void OmnitechKeyDiversification::initDiversification(
    std::vector<unsigned char> identifier, int /*AID*/, std::shared_ptr<Key> /*key*/,
...
OmnitechKeyDiversification::getDiversifiedKey(std::shared_ptr<Key> key,
                                              std::vector<unsigned char> diversify)
{
    ...
    std::vector<unsigned char> r = DESFireCrypto::sam_CBC_send(
        vkeydata, iv,
        std::vector<unsigned char>(diversify.begin(), diversify.begin() + 8));
    std::vector<unsigned char> r2 = DESFireCrypto::sam_CBC_send(
        vkeydata, iv,
        std::vector<unsigned char>(diversify.begin() + 8, diversify.begin() + 16));
