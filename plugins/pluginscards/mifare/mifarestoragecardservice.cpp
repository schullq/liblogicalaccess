    ...
    if (nbSectors >= 1)
    {
        ...
        if (behaviorFlags & CB_AUTOSWITCHAREA)
        {
            getMifareChip()->getMifareCommands()->writeSectors(
                ...
                dataSectors, mAiToUse->keyA, mAiToUse->keyB, mAiToUse->sab, mAiToUse->gpb,
        ...
        else
        {
            getMifareChip()->getMifareCommands()->writeSector(
                ...
                mAiToUse->keyB, mAiToUse->sab, mAiToUse->gpb,
                mAiToWrite ? &(mAiToWrite->sab) : NULL,
                mAiToWrite ? mAiToWrite->keyA : std::shared_ptr<MifareKey>(),
                mAiToWrite ? mAiToWrite->keyB : std::shared_ptr<MifareKey>());
        }
...
MifareStorageCardService::readData(std::shared_ptr<Location> location,
                                   ...
                                   CardBehavior behaviorFlags)
{
    ...
    if (nbSectors >= 1)
    {
        ...
        {
            ret.insert(ret.end(), dataSectors.begin() + mLocation->byte,
                       dataSectors.begin() + mLocation->byte + length);
        }
    }
    return ret;
}
                                                      ...
                                                      void *data, size_t dataLength)
{
    ...
    if (dataLength >= 16)
    {
        ...
        std::vector<unsigned char> vdata =
            getMifareChip()->getMifareCommands()->readBinary(
                static_cast<unsigned char>(
                    getMifareChip()->getMifareCommands()->getSectorStartBlock(
                        mLocation->sector) +
