...
{
void MifareNFCTagCardService::writeNDEF(
    std::shared_ptr<logicalaccess::NdefMessage> records)
{
    ...
    std::shared_ptr<logicalaccess::MifareLocation> location(
    ...
    storage->writeData(location, std::shared_ptr<logicalaccess::AccessInfo>(), aiToWrite,
...
std::shared_ptr<logicalaccess::NdefMessage> MifareNFCTagCardService::readNDEF()
{
    ...
    std::vector<unsigned char> data =
        storage->readData(location, std::shared_ptr<logicalaccess::MifareAccessInfo>(),
                          48, CB_AUTOSWITCHAREA);
    try
    {
        location->sector                 = 2;
        std::vector<unsigned char> data2 = storage->readData(
            location, std::shared_ptr<logicalaccess::MifareAccessInfo>(), 48,
            CB_AUTOSWITCHAREA);
