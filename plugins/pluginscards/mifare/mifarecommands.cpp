                                              ...
                                              std::shared_ptr<MifareKey> madKeyA)
{
    ...
    std::vector<unsigned char> madbuf =
        readSector(0, 1, madKeyA, std::shared_ptr<MifareKey>(), sab);
    if (!madbuf.size())
    {
...
MifareCommands::readSector(int sector, int start_block, std::shared_ptr<MifareKey> keyA,
...
void MifareCommands::writeSector(
    int sector, int start_block, const std::vector<unsigned char> &buf,
    std::shared_ptr<MifareKey> keyA, std::shared_ptr<MifareKey> keyB,
    const MifareAccessInfo::SectorAccessBits &sab, unsigned char userbyte,
    MifareAccessInfo::SectorAccessBits *newsab, std::shared_ptr<MifareKey> newkeyA,
    std::shared_ptr<MifareKey> newkeyB)
{
    ...
    for (int i = start_block; i < getNbBlocks(sector); i++)
    {
        ...
        updateBinary(
            static_cast<unsigned char>(getSectorStartBlock(sector) + i),
            std::vector<unsigned char>(buf.begin() + retlen, buf.begin() + retlen + 16));
        retlen += 16;
    }
...
std::vector<unsigned char> MifareCommands::readSectors(
    int start_sector, int stop_sector, int start_block, std::shared_ptr<MifareKey> keyA,
    std::shared_ptr<MifareKey> keyB, const MifareAccessInfo::SectorAccessBits &sab)
{
    ...
    for (int i = start_sector; i <= stop_sector; ++i)
    {
        ...
        std::vector<unsigned char> data =
            readSector(i, startBlockSector, keyA, keyB, sab);
...
void MifareCommands::writeSectors(
    int start_sector, int stop_sector, int start_block,
    const std::vector<unsigned char> &buf, std::shared_ptr<MifareKey> keyA,
    std::shared_ptr<MifareKey> keyB, const MifareAccessInfo::SectorAccessBits &sab,
    ...
    std::shared_ptr<MifareKey> newkeyA, std::shared_ptr<MifareKey> newkeyB)
{
    ...
    {
        int startBlockSector = (i == start_sector) ? start_block : 0;
        std::vector<unsigned char> tmp(buf.begin() + offset,
                                       buf.begin() + offset +
                                           (getNbBlocks(i) - startBlockSector) * 16);
