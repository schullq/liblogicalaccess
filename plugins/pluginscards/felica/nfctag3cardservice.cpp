...
void NFCTag3CardService::writeNDEF(std::shared_ptr<logicalaccess::NdefMessage> records)
{
    ...
    std::shared_ptr<logicalaccess::FeliCaCommands> cmd =
        getFeliCaChip()->getFeliCaCommands();
    EXCEPTION_ASSERT_WITH_LOG(cmd, CardException,
                              "FeliCa commands not implemented on this reader.");
    std::shared_ptr<logicalaccess::FeliCaLocation> location(
        new logicalaccess::FeliCaLocation);
    ...
    if (data.size() > 0)
        storage->writeData(location, std::shared_ptr<logicalaccess::AccessInfo>(),
                           std::shared_ptr<logicalaccess::AccessInfo>(), data,
                           CB_AUTOSWITCHAREA);
}
...
std::shared_ptr<logicalaccess::NdefMessage> NFCTag3CardService::readNDEF()
{
    ...
    {
        std::vector<unsigned char> data =
            storage->readData(location, std::shared_ptr<logicalaccess::AccessInfo>(),
