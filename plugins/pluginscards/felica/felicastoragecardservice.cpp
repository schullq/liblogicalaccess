        ...
        while (i < data.size())
        {
            cmd->write(icLocation->code, icLocation->block + (unsigned int)(i / 16),
                       std::vector<unsigned char>(
                           data.cbegin() + i,
                           (i + 16 < data.size()) ? data.begin() + i + 16 : data.cend()));
            i += 16;
        }
    }
    else
...
std::vector<unsigned char>
FeliCaStorageCardService::readData(std::shared_ptr<Location> location,
                                   ...
                                   CardBehavior cardBehavior)
{
    ...
    if ((cardBehavior & CB_AUTOSWITCHAREA) == CB_AUTOSWITCHAREA)
    {
        ...
        while (i < length)
        {
            std::vector<unsigned char> bdata = cmd->read(
                icLocation->code, (unsigned short)(icLocation->block + (i / 16)));
            if (bdata.size() == 0)
                break;
...
unsigned int FeliCaStorageCardService::readDataHeader(std::shared_ptr<Location>,
                                                      std::shared_ptr<AccessInfo>, void *,
                                                      size_t)
{
    return 0;
}
