    ...
    if (format)
    {
        ...
        if (pLocation)
        {
            if (dataLengthBits > 0)
            {
                ...
                {
                    unsigned int pos = 0;
                    BitHelper::writeToBit(
                        formatBuf, length, &pos, &getChip()->getChipIdentifier()[0],
                        getChip()->getChipIdentifier().size(),
                        static_cast<unsigned int>(getChip()->getChipIdentifier().size() *
                                                  8),
                        static_cast<unsigned int>(pLocation->bit), dataLengthBits);
