...
STidSTRReaderCardAdapter::adaptCommand(const std::vector<unsigned char> &command)
{
    LOG(LogLevel::COMS) << "Sending command command " << BufferHelper::getHex(command)
    ...
    cmd.push_back(SOF);
    std::vector<unsigned char> commandEncapsuled = sendMessage(
        commandCode, std::vector<unsigned char>(command.begin() + 2, command.end()));
...
std::vector<unsigned char>
STidSTRReaderCardAdapter::sendMessage(unsigned short commandCode,
                                      const std::vector<unsigned char> &command)
{
    ...
    if ((readerConfig->getCommunicationMode() & STID_CM_CIPHERED) == STID_CM_CIPHERED)
    {
        ...
        {
            d_lastIV = std::vector<unsigned char>(encProcessedMsg.end() - 16,
                                                  encProcessedMsg.end());
        }
...
std::vector<unsigned char>
STidSTRReaderCardAdapter::sendCommand(unsigned short commandCode,
                                      const std::vector<unsigned char> &command,
                                      long int timeout)
{
    LOG(LogLevel::COMS) << "Sending message with command code {0x" << std::hex
...
std::vector<unsigned char>
STidSTRReaderCardAdapter::adaptAnswer(const std::vector<unsigned char> &answer)
{
    ...
    std::vector<unsigned char> data =
        std::vector<unsigned char>(answer.begin() + 5, answer.begin() + 5 + messageSize);
...
std::vector<unsigned char>
STidSTRReaderCardAdapter::calculateHMAC(const std::vector<unsigned char> &buf) const
{
    // HMAC-SHA-1
...
std::vector<unsigned char>
STidSTRReaderCardAdapter::receiveMessage(const std::vector<unsigned char> &data,
                                         unsigned char &statusCode)
{
    ...
    if ((readerConfig->getCommunicationMode() & STID_CM_SIGNED) == STID_CM_SIGNED)
    {
        ...
        EXCEPTION_ASSERT_WITH_LOG(
            std::vector<unsigned char>(data.end() - 10, data.end()) ==
    ...
    if ((readerConfig->getCommunicationMode() & STID_CM_CIPHERED) == STID_CM_CIPHERED)
    {
        ...
        std::vector<unsigned char> iv =
            std::vector<unsigned char>(tmpData.end() - 16, tmpData.end());
    ...
    std::vector<unsigned char> plainData = std::vector<unsigned char>(
        tmpData.begin() + offset, tmpData.begin() + offset + msglength);
    offset += msglength;
