...
std::vector<unsigned char> DESFireEV1STidSTRCommands::scanDESFire()
{
    ...
    std::vector<unsigned char> r =
        getSTidSTRReaderCardAdapter()->sendCommand(0x0001, std::vector<unsigned char>());
...
unsigned int DESFireEV1STidSTRCommands::getFreeMem()
{
    ...
    std::vector<unsigned char> r =
        getSTidSTRReaderCardAdapter()->sendCommand(0x006E, std::vector<unsigned char>());
    ...
    std::vector<unsigned char> isoDFName)
{
    ...
    {
        getDESFireChip()->getCrypto()->setKey(aid, i,
                                              DESFireCrypto::getDefaultKey(cryptoMethod));
    }
}
                                               ...
                                               DESFireKeyType &keyType)
{
    ...
    std::vector<unsigned char> r =
        getSTidSTRReaderCardAdapter()->sendCommand(0x0045, std::vector<unsigned char>());
...
unsigned char DESFireEV1STidSTRCommands::getKeyVersion(unsigned char keyno)
{
    ...
    std::vector<unsigned char> r =
        getSTidSTRReaderCardAdapter()->sendCommand(0x0064, command);
...
std::vector<unsigned char> DESFireEV1STidSTRCommands::getCardUID()
{
    ...
    std::vector<unsigned char> r =
        getSTidSTRReaderCardAdapter()->sendCommand(0x0051, std::vector<unsigned char>());
...
void DESFireEV1STidSTRCommands::createValueFile(
    ...
    unsigned int lowerLimit, unsigned int upperLimit, unsigned int value,
...
void DESFireEV1STidSTRCommands::createLinearRecordFile(unsigned char fileno,
                                                       EncryptionMode comSettings,
                                                       DESFireAccessRights accessRights,
                                                       unsigned int fileSize,
...
void DESFireEV1STidSTRCommands::createCyclicRecordFile(unsigned char fileno,
                                                       EncryptionMode comSettings,
                                                       DESFireAccessRights accessRights,
                                                       unsigned int fileSize,
                                                       unsigned int maxNumberOfRecords)
...
std::vector<unsigned char>
DESFireEV1STidSTRCommands::iso_readRecords(unsigned short /*fid*/,
...
void DESFireEV1STidSTRCommands::iso_appendrecord(
    const std::vector<unsigned char> & /*data*/, unsigned short /*fid*/)
{
    THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException,
                             "Function not available with this reader.");
}
...
std::vector<unsigned char>
DESFireEV1STidSTRCommands::iso_getChallenge(unsigned int /*length*/)
{
    THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException,
                             "Function not available with this reader.");
    return std::vector<unsigned char>();
...
std::vector<unsigned char> DESFireEV1STidSTRCommands::iso_internalAuthenticate(
    DESFireISOAlgorithm /*algorithm*/, bool /*isMasterCardKey*/, unsigned char /*keyno*/,
...
void DESFireEV1STidSTRCommands::loadKey(std::shared_ptr<DESFireKey> key)
{
    ...
    if (std::dynamic_pointer_cast<ComputerMemoryKeyStorage>(key_storage))
    {
        ...
        loadKey(std::vector<unsigned char>(keydata, keydata + key->getLength()),
                key->getKeyDiversification() != NULL, true);
    }
...
void DESFireEV1STidSTRCommands::loadKey(std::vector<unsigned char> key,
                                        bool /*diversify*/, bool /*isVolatile*/)
{
                                             ...
                                             unsigned char keyindex)
{
    LOG(LogLevel::INFOS) << "Authenticating from location... key location {"
                         << DESFireEV1STidSTRCommands::STidKeyLocationTypeStr(keylocation)
                         << "} key number {0x" << std::hex << keyno << std::dec << "("
                         << keyno << ")}, key type {"
...
void DESFireEV1STidSTRCommands::authenticate(unsigned char keyno)
{
    std::shared_ptr<DESFireKey> key =
        getDESFireChip()->getCrypto()->getKey(d_currentAid, keyno);
...
std::vector<unsigned char> DESFireEV1STidSTRCommands::readData(unsigned char fileno,
                                                               unsigned int offset,
                                                               unsigned int length,
                                                               EncryptionMode mode)
{
    ...
    for (size_t i = 0; i < length; i += stidMaxDataSize)
    {
        ...
        std::vector<unsigned char> data =
            getSTidSTRReaderCardAdapter()->sendCommand(0x00BD, command);
...
std::vector<unsigned char> DESFireEV1STidSTRCommands::readRecords(unsigned char fileno,
                                                                  unsigned int offset,
                                                                  ...
                                                                  EncryptionMode mode)
{
    ...
    std::vector<unsigned char> result =
        getSTidSTRReaderCardAdapter()->sendCommand(0x00BB, command);
...
void DESFireEV1STidSTRCommands::changeFileSettings(unsigned char fileno,
                                                   EncryptionMode comSettings,
                                                   DESFireAccessRights accessRights,
                                                   bool plain)
{
    ...
    DESFireCommands::DESFireCardVersion &dataVersion)
{
    ...
    std::vector<unsigned char> result =
        getSTidSTRReaderCardAdapter()->sendCommand(0x0060, std::vector<unsigned char>());
...
std::vector<unsigned int> DESFireEV1STidSTRCommands::getApplicationIDs()
{
    ...
    std::vector<unsigned char> result =
        getSTidSTRReaderCardAdapter()->sendCommand(0x006A, std::vector<unsigned char>());

    unsigned nbaids = result[0];
    for (size_t i = 0; i < nbaids; ++i)
    {
        std::vector<unsigned char> aid(3);
        std::copy(result.begin() + 1 + (i * 3), result.begin() + 1 + (i * 3) + 3,
                  aid.begin());
...
std::vector<unsigned char> DESFireEV1STidSTRCommands::getFileIDs()
{
    ...
    std::vector<unsigned char> result =
        getSTidSTRReaderCardAdapter()->sendCommand(0x006F, std::vector<unsigned char>());
                                                ...
                                                FileSetting &fileSetting)
{
    ...
    std::vector<unsigned char> result =
        getSTidSTRReaderCardAdapter()->sendCommand(0x00F5, command);
...
void DESFireEV1STidSTRCommands::iso_selectApplication(
    std::vector<unsigned char> /*isoaid*/)
{
    LOG(LogLevel::ERRORS) << "Function not available with this reader.";
    THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException,
                             "Function not available with this reader.");
...
std::string DESFireEV1STidSTRCommands::STidKeyLocationTypeStr(STidKeyLocationType t)
{
    switch (t)
    {
    case STID_DF_KEYLOC_RAM:
        return "STID_DF_KEYLOC_RAM";
    case STID_DF_KEYLOC_EEPROM:
        return "STID_DF_KEYLOC_EEPROM";
    case STID_DF_KEYLOC_INDEXED:
        return "STID_DF_KEYLOC_INDEXED";
    default:
        return "Unknown";
    }
}
