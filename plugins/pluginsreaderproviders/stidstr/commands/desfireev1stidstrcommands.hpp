...
class LIBLOGICALACCESS_API DESFireEV1STidSTRCommands : public DESFireEV1Commands
{
  public:
    ...
    virtual void writeData(unsigned char fileno, unsigned int offset,
                           const std::vector<unsigned char> &data, EncryptionMode mode);
     ...
     */
    std::shared_ptr<STidSTRReaderCardAdapter> getSTidSTRReaderCardAdapter()
    {
        return std::dynamic_pointer_cast<STidSTRReaderCardAdapter>(
            getReaderCardAdapter());
    };

    void setProfile(std::shared_ptr<DESFireProfile> profile)
    {
        d_profile = profile;
    };
