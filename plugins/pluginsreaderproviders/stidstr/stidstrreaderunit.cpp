    ...
    : ReaderUnit(READER_STIDSTR)
{
    ...
    setDefaultReaderCardAdapter(std::shared_ptr<STidSTRReaderCardAdapter>(
        new STidSTRReaderCardAdapter(STID_CMD_READER)));
...
std::shared_ptr<Chip> STidSTRReaderUnit::scan14443A()
{
    ...
    std::vector<unsigned char> response =
        getDefaultSTidSTRReaderCardAdapter()->sendCommand(0x0002,
    ...
    if (response.size() > 0)
    {
        ...
        if (haveCard && response.size() > 4)
        {
            ...
            if (uidLen > 0)
            {
                std::vector<unsigned char> uid = std::vector<unsigned char>(
                    response.begin() + 5, response.begin() + 5 + uidLen);
...
std::shared_ptr<Chip> STidSTRReaderUnit::scan14443B()
{
    ...
    std::shared_ptr<Chip> chip;
    std::vector<unsigned char> response =
        getDefaultSTidSTRReaderCardAdapter()->sendCommand(0x0009,
                                                          std::vector<unsigned char>());
    if (response.size() > 0)
    {
        bool haveCard = (response[0] == 0x01);
        if (haveCard && response.size() > 4)
        {
            ...
            {
                std::vector<unsigned char> uid = std::vector<unsigned char>(
                    response.begin() + 2, response.begin() + 5 + uidLen);
...
void STidSTRReaderUnit::authenticateHMAC()
{
    ...
    std::vector<unsigned char> rndB2 =
        std::vector<unsigned char>(buf3.begin(), buf3.begin() + 16);
    EXCEPTION_ASSERT_WITH_LOG(rndB2 == rndB, LibLogicalAccessException,
                              "Cannot negotiate the session: RndB'' != RndB.");
    std::vector<unsigned char> rndC =
        std::vector<unsigned char>(buf3.begin() + 16, buf3.begin() + 16 + 16);
...
void STidSTRReaderUnit::authenticateAES()
{
    ...
    openssl::AESSymmetricKey aeskey = openssl::AESSymmetricKey::createFromData(
        std::vector<unsigned char>(keydata, keydata + key->getLength()));
    openssl::AESInitializationVector aesiv =
        openssl::AESInitializationVector::createNull();
...
std::vector<unsigned char>
STidSTRReaderUnit::authenticateReader2(const std::vector<unsigned char> &data)
{
...
void STidSTRReaderUnit::ResetAuthenticate()
{
    ...
    getDefaultSTidSTRReaderCardAdapter()->sendCommand(0x000D,
                                                      std::vector<unsigned char>());
}

void STidSTRReaderUnit::ChangeReaderKeys(const std::vector<unsigned char> &key_hmac,
                                         const std::vector<unsigned char> &key_aes)
{
...
STidSTRReaderUnit::STidSTRInformation STidSTRReaderUnit::getReaderInformaton()
{
    ...
    std::vector<unsigned char> response =
        getDefaultSTidSTRReaderCardAdapter()->sendCommand(0x0008,
                                                          std::vector<unsigned char>());
                                             ...
                                             bool &swChanged)
{
    ...
    std::vector<unsigned char> response =
        getDefaultSTidSTRReaderCardAdapter()->sendCommand(0x000C,
                                                          std::vector<unsigned char>());
...
void STidSTRReaderUnit::loadSKB()
{
    ...
    std::vector<unsigned char> response =
        getDefaultSTidSTRReaderCardAdapter()->sendCommand(
            0x000E, std::vector<unsigned char>(), statusCode);

    EXCEPTION_ASSERT_WITH_LOG(response.size() == 0, LibLogicalAccessException,
                              "Unable to load the SKB values. An unknown error occured.");
}
