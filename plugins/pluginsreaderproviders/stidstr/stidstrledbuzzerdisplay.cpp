...
{
    LOG(LogLevel::INFOS) << "Sending LED/Buzzer command for: red{" << d_red_led
                         << "} green{" << d_green_led << "} buzzer{" << d_buzzer
                         << "}...";
    std::vector<unsigned char> data;
    data.push_back(
        static_cast<unsigned char>((d_red_led ? 2 : 0) | (d_green_led ? 1 : 0)));
