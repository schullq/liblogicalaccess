    ...
    do
    {
        ...
        if (d_buffer.size() >= 4)
        {
            ...
            if (d_buffer.size() >= exceptedlen)
            {
                ...
                std::vector<unsigned char> bufnoc =
                    std::vector<unsigned char>(buf.begin(), buf.end() - 1);
                ...
                if (ret.size() != 0 && buf[1] == Device::HID &&
                    buf[2] == HidCommand::BADGE)
                {
                    // save the badge
                    if (d_badges.size() <= 10)
...
RplethDataTransport::sendCommand(const std::vector<unsigned char> &command,
                                 long int timeout)
{
