    ...
    : ISO7816ReaderUnit(READER_RPLETH)
{
    ...
    setDefaultReaderCardAdapter(
        std::shared_ptr<RplethReaderCardAdapter>(new RplethReaderCardAdapter()));
    ...
    if (getRplethConfiguration()->getMode() == RplethMode::PROXY)
    {
        ...
        try
        {
            answer = getDefaultRplethReaderCardAdapter()->sendRplethCommand(
                command, true, maxwait + 2000); // Give More Time To Answer
        }
        ...
        if (answer.size() > 0)
        {
            ...
            std::vector<unsigned char> csn = std::vector<unsigned char>(
                answer.begin() + 1 + ctypelen + 1, answer.end());
...
RplethReaderUnit::getCsn(const std::vector<unsigned char> &trame)
{
