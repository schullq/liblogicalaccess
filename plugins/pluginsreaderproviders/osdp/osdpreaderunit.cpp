    ...
    if (chip)
    {
        ...
        if (type == "DESFire" || type == "DESFireEV1")
        {
            ...
            rca.reset(new OSDPReaderCardAdapter(
                ...
                std::shared_ptr<DESFireISO7816ResultChecker>(
                    new DESFireISO7816ResultChecker())));
    ...
    else
    {
        do
        {
            ...
            if (poll->getCommandsType() == OSDPCommandsType::RAW)
            {
                ...
                LOG(LogLevel::INFOS)
                    << "ATQA card type detected:" << BufferHelper::getHex(m_currencard)
                    << "}";
...
bool OSDPReaderUnit::waitRemoval(unsigned int maxwait)
{
    ...
    do
    {
        ...
        if (poll->getCommandsType() == OSDPCommandsType::RAW)
        {
            ...
            LOG(LogLevel::INFOS)
                << "ATQA card type detected:" << BufferHelper::getHex(currentcard) << "}";
    ...
    {
        d_insertedChip->getCommands()->setReaderCardAdapter(
        ...
        if (result->getCommandsType() == OSDPCommandsType::XRD)
        {
            ...
            if (result->getCommandsType() != OSDPCommandsType::ACK)
                THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException,
                                         "Impossible to set Profile 0x00");
        }
    }
