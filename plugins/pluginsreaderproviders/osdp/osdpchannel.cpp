...
OSDPChannel::OSDPChannel()
    ...
    , m_reply_type(OSDPCommandsType::NOCMD)
    , m_securityBlockType(OSDPSecureChannelType::NOSCS)
    , m_securityBlockData(std::vector<unsigned char>())
{
}
...
void OSDPChannel::unPackage(std::vector<unsigned char> result)
{
    ...
    if (isSCB)
    {
        ...
        std::vector<unsigned char> securityBlock(result.begin() + index,
    ...
    if (isSCB && getSecurityBlockType() >= OSDPSecureChannelType::SCS_15 &&
        getSecurityBlockType() <= OSDPSecureChannelType::SCS_18)
    {
        dataLength -= 0x04;
        getSecureChannel()->verifyMAC(
            std::vector<unsigned char>(result.begin(), result.end() - 2));
    }
    if (dataLength > 0)
    {
        std::vector<unsigned char> data(result.begin() + index,
                                        result.begin() + index + dataLength);
        if (isSCB && (getSecurityBlockType() == OSDPSecureChannelType::SCS_17 ||
                      getSecurityBlockType() == OSDPSecureChannelType::SCS_18))
        {
