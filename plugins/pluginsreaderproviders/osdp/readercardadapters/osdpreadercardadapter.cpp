...
OSDPReaderCardAdapter::sendCommand(const std::vector<unsigned char> &command,
                                   long timeout)
{
    ...
    if (channel->isSCB)
    {
        ...
        channel->setSecurityBlockType(
            OSDPSecureChannelType::SCS_17); // Enable MAC and Data Security
    }
    ...
    if (result->getCommandsType() != OSDPCommandsType::XRD ||
        (result->getCommandsType() == OSDPCommandsType::XRD &&
