    ...
    : ReaderUnit(READER_IDONDEMAND)
{
    ...
    setDefaultReaderCardAdapter(
        std::shared_ptr<IdOnDemandReaderCardAdapter>(new IdOnDemandReaderCardAdapter()));
...
void IdOnDemandReaderUnit::authenticateSDK(std::string authCode)
{
    ...
    getDefaultIdOnDemandReaderCardAdapter()->sendCommand(
        std::vector<unsigned char>(strcmd.begin(), strcmd.end()));
}
...
bool IdOnDemandReaderUnit::write()
{
    ...
    try
    {
        ...
        getDefaultIdOnDemandReaderCardAdapter()->sendCommand(
            std::vector<unsigned char>(strcmd.begin(), strcmd.end()), 4000);
    }
    catch (CardException &e)
    {
