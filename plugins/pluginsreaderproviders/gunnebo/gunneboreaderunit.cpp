    ...
    : ReaderUnit(READER_GUNNEBO)
{
    ...
    setDefaultReaderCardAdapter(
        std::shared_ptr<GunneboReaderCardAdapter>(new GunneboReaderCardAdapter()));
...
GunneboReaderUnit::processCardId(std::vector<unsigned char> &rawSerialData)
{
    ...
    if (rawSerialData.size() >= 2)
    {
        ...
        {
            std::vector<unsigned char> hexSerialData =
                BufferHelper::fromHexString(strSerialData);
