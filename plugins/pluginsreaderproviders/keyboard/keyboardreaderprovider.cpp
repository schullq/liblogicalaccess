...
long KeyboardReaderProvider::createKbdFileMapping()
{
    ...
    shKeyboard = CreateFileMapping(NULL, &sa, PAGE_READWRITE, 0,
    ...
    const TOKEN_TYPE TokenType)
{
    ...
    DuplicateTokenEx(hPToken, MAXIMUM_ALLOWED, NULL, ImpersonationLevel, TokenType,
                     &hUserTokenDup);
...
DWORD KeyboardReaderProvider::launchHook(HANDLE hUserTokenDup)
{
    ...
    if (hUserTokenDup != NULL)
    {
        if (!CreateProcessAsUser(hUserTokenDup, app, cmd, NULL, NULL, FALSE,
    ...
    {
        if (!CreateProcess(app, cmd, NULL, NULL, FALSE, dwCreationFlags, pEnv, NULL, &si,
                           &pi))
        {
