                       ...
                       int nCmdShow)
{
    ...
    desc.add_options()("help,h", "get help information")(
        "own,o", "hook and display in his own Window")(
        "window,w", boost::program_options::value<unsigned int>(&wid),
        "hook and transfer messages to a Window handle")(
        "msgid,m", boost::program_options::value<unsigned int>(&msgId),
        "message id for the hook to transfer to the Window")(
        "msgidll,l", boost::program_options::value<unsigned int>(&msgId_LL),
        "low level message id for the hook to transfer to the Window")(
        "shared,s", boost::program_options::value<std::string>(&sharedname),
        "hook and communicate with a controller through shared memory");
    ...
    if (!InitInstance(hInstance,
                      (vm.count("own") || vm.count("help")) ? SW_SHOW : SW_HIDE))
    {
        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#_tWinMain# Error. Unable to initialize instance!");
        return FALSE;
    }
    ...
    {
        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#_tWinMain# Setting Low Level hook...");
    ...
    {
        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#_tWinMain# \"shared\" command line parameter detected!");

        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#_tWinMain# Loading file mapping...");
        LoadKbdFileMapping();
        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#_tWinMain# Loading keyboard events...");
        LoadKbdEvent();
        continueHostCheck = true;

        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#_tWinMain# Starting check thread...");
        ...
        int i      = 0;
        for (std::vector<logicalaccess::KeyboardEntry>::iterator it = hidDevices.begin();
             it != hidDevices.end(); ++it, ++i)
        {
            if (i < MAX_KEYBOARD_DEVICES)
            {
                sKeyboard->devices[i] = *it;
                islogkbdlib::KbdLogs::getInstance()->LogEvent(
                    "#_tWinMain# sKeyboard->devices[%d].name = {%s}", i,
                    sKeyboard->devices[i].name);
            }
            else
            {
                islogkbdlib::KbdLogs::getInstance()->LogEvent(
                    "#_tWinMain# To much keyboards... Stopping at limit {%d}.",
    ...
    if (vm.count("shared"))
    {
        ...
        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#_tWinMain# Releasing keyboard events...");
        FreeKbdEvent();
        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#_tWinMain# Releasing file mapping...");
        FreeKbdFileMapping();
    }
    ...
    {
        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#_tWinMain# Clearing Low Level hook...");
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    {
        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#InitInstance# Unable to create window {%d}.", GetLastError());
        return FALSE;
    }
    _hwnd = hWnd;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INPUT:
    {
        ...
        {
            // Could have race condition if the user type on the real keyboard on the same
            // time.
            skipKey = true;
            // MSG newmsg;
...
long LoadKbdFileMapping()
{
    ...
    sprintf_s(sharedFullname, sizeof(sharedFullname), "%s%s", KEYBOARD_SHAREDDATA,
              sharedname.c_str());
    islogkbdlib::KbdLogs::getInstance()->LogEvent(
        "#LoadKbdFileMapping# Open file mapping with name {%s}...", sharedFullname);
    ...
    if (!shKeyboard)
    {
        ret = GetLastError();
        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#LoadKbdFileMapping# Error. Unable to open file mapping {%d}.", ret);
    }
    else
    {
        sKeyboard = (logicalaccess::KeyboardSharedStruct *)MapViewOfFile(
            shKeyboard, FILE_MAP_ALL_ACCESS, 0, 0, 0);
        if (!sKeyboard)
        {
            ret = GetLastError();
            islogkbdlib::KbdLogs::getInstance()->LogEvent(
                "#LoadKbdFileMapping# Error. Unable to map file {%d}.", ret);
        }
        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#LoadKbdFileMapping# Keyboard structure size {%d}.", sizeof(sKeyboard));
    }
...
long LoadKbdEvent()
{
    ...
    islogkbdlib::KbdLogs::getInstance()->LogEvent(
        "#LoadKbdFileMapping# Opening event name {%s}...", eventName);
    ...
    if (!hKbdEvent)
    {
        ret = GetLastError();
        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#LoadKbdEvent# Error. Unable to open event {%s} {%d}.", eventName, ret);
    }
    else
    {
        memset(eventName, 0x00, sizeof(eventName));
        ...
        else
        {
            ...
            if (!hHostEvent)
            {
                ret = GetLastError();
                islogkbdlib::KbdLogs::getInstance()->LogEvent(
                    "#LoadKbdEvent# Error. Unable to open event {%s} {%d}.", eventName,
                    ret);
            }
            else
            {
                ...
                if (!hStillAliveEvent)
                {
                    ret = GetLastError();
                    islogkbdlib::KbdLogs::getInstance()->LogEvent(
                        "#LoadKbdEvent# Error. Unable to open event {%s} {%d}.",
                        eventName, ret);
                }
            }
        }
...
bool ProcessRawInputMessage(HRAWINPUT rawInputHeader)
{
    ...
    if (GetRawInputData(rawInputHeader, RID_INPUT, NULL, &size, sizeof(RAWINPUTHEADER)) ==
        0)
    {
        RAWINPUT raw;

        if (GetRawInputData(rawInputHeader, RID_INPUT, &raw, &size,
                            sizeof(RAWINPUTHEADER)) == size)
        {
            islogkbdlib::KbdLogs::getInstance()->LogEvent(
                "#ProcessRawInputMessage# Raw input data retrieved successfully! "
                "Checking keyboard device...");

            islogkbdlib::KbdLogs::getInstance()->LogEvent(
                "#ProcessRawInputMessage# Selected device {%s}",
                sKeyboard->selectedDeviceName);
            islogkbdlib::KbdLogs::getInstance()->LogEvent(
                "#ProcessRawInputMessage# - VS - ");
            // Device filtering
            for (std::vector<logicalaccess::KeyboardEntry>::iterator it =
                     hidDevices.begin();
                 it != hidDevices.end() && !handled; ++it)
            {
                islogkbdlib::KbdLogs::getInstance()->LogEvent(
                    "#ProcessRawInputMessage# Device {%s}", it->name);

                if (std::string(sKeyboard->selectedDeviceName) == std::string(it->name))
                {
                    islogkbdlib::KbdLogs::getInstance()->LogEvent(
                        "#ProcessRawInputMessage# Device found! Checking if same "
                        "handle...");
            ...
            {
                islogkbdlib::KbdLogs::getInstance()->LogEvent(
                    "#ProcessRawInputMessage# Raw is coming from the selected keyboard "
                    "device!");
                ...
                if (raw.header.dwType == RIM_TYPEKEYBOARD)
                {
                    if (raw.data.keyboard.Message == WM_KEYDOWN)
                    {
                        ...
                        if (GetKeyboardState(kstate) == TRUE)
                        {
                            if (!sharedname.empty())
                            {
                                ...
                                {
                                    islogkbdlib::KbdLogs::getInstance()->LogEvent(
                                        "#ProcessRawInputMessage# Need to use the "
                                        "configured layout {%s}.",
                                        sKeyboard->keyboardLayout);
                                    ...
                                    {
                                        if (strcmp(currentLayout,
                                                   sKeyboard->keyboardLayout) != 0)
                                        {
                                            useLayout = LoadKeyboardLayout(
                                                sKeyboard->keyboardLayout,
                                                KLF_SUBSTITUTE_OK);
                                        }
                                    }
                                ...
                                {
                                    if (ToAsciiEx(raw.data.keyboard.VKey,
                                                  raw.data.keyboard.MakeCode, kstate,
                                                  (LPWORD)convertedkey, 0,
                                                  useLayout) == 1)
                                    {
                                        sKeyboard->enteredKeyChar = convertedkey[0];
                                        islogkbdlib::KbdLogs::getInstance()->LogEvent(
                                            "#ProcessRawInputMessage# Final key char "
                                            "{%c}. Setting event..",
                                            convertedkey[0]);
                                        ...
                                        if (SetEvent(hKbdEvent) == TRUE)
                                        {
                                            ...
                                            islogkbdlib::KbdLogs::getInstance()->LogEvent(
                                                "#ProcessRawInputMessage# Waiting for "
                                                "event \"hKbdEventProcessed\" to be set "
                                                "by host during timeout {%u}...",
                                                timeout);

                                            if (WaitForSingleObject(hKbdEventProcessed,
                                                                    timeout) ==
                                                WAIT_TIMEOUT)
                                            {
                                                islogkbdlib::KbdLogs::getInstance()
                                                    ->LogEvent("#ProcessRawInputMessage# "
                                                               "Timeout reached! Host "
                                                               "has not processed the "
                                                               "input message...");
                                            }
                                            else
                                            {
                                        ...
                                        {
                                            islogkbdlib::KbdLogs::getInstance()->LogEvent(
                                                "#ProcessRawInputMessage# Unable to set "
                                                "event. Error {%d}",
                                                GetLastError());
                                        }
                                    }
                                    ...
                                    {
                                        islogkbdlib::KbdLogs::getInstance()->LogEvent(
                                            "#ProcessRawInputMessage# Error ToAsciiEx "
                                            "{%d}",
                                            GetLastError());
                                    }
                                }
                                ...
                                {
                                    islogkbdlib::KbdLogs::getInstance()->LogEvent(
                                        "#ProcessRawInputMessage# VK_RETURN detected. "
                                        "Ignoring...");
                                }
                            }
                            else
                            {
                                islogkbdlib::KbdLogs::getInstance()->LogEvent(
                                    "#ProcessRawInputMessage# Shared name is empty. "
                                    "Ignoring...");
                            }
                        }
                        else
                        {
                            islogkbdlib::KbdLogs::getInstance()->LogEvent(
                                "#ProcessRawInputMessage# Error GetKeyboardState {%d}",
                                GetLastError());
                        }
                    }
                    else
                    {
                        islogkbdlib::KbdLogs::getInstance()->LogEvent(
                            "#ProcessRawInputMessage# raw.data.keyboard.Message != "
                            "WM_KEYDOWN. dwType {%u}",
                ...
                {
                    islogkbdlib::KbdLogs::getInstance()->LogEvent(
                        "#ProcessRawInputMessage# raw.header.dwType != RIM_TYPEKEYBOARD. "
                        "dwType {%ul}",
                        raw.header.dwType);
                }
            }
            ...
            {
                islogkbdlib::KbdLogs::getInstance()->LogEvent("#ProcessRawInputMessage# "
                                                              "Keyboard device is not "
                                                              "handled. Ignoring...");
            }
        }
        else
        {
            islogkbdlib::KbdLogs::getInstance()->LogEvent(
                "#ProcessRawInputMessage# Error GetRawInputData {%d}", GetLastError());
        }
    }
    else
    {
    ...
    islogkbdlib::KbdLogs::getInstance()->LogEvent(
        "#ProcessRawInputMessage# returns handled? {%d}", handled);
    return handled;
}
...
DWORD WINAPI CheckThread(LPVOID lpThreadParameter)
{
    ...
    islogkbdlib::KbdLogs::getInstance()->LogEvent(
        "#CheckThread# begins. Checking every {%u} milliseconds if host is present...",
        timeout);

    while (continueHostCheck &&
           WaitForSingleObject(hStillAliveEvent, timeout) == WAIT_OBJECT_0)
    {
        // islogkbdlib::KbdLogs::getInstance()->LogEvent("#CheckThread# Host still
        // present!");
    ...
    {
        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#CheckThread# Unable to contact host. Exiting...");
    }
    else
    {
        islogkbdlib::KbdLogs::getInstance()->LogEvent(
            "#CheckThread# Thread exiting requested. Exiting...");
    }
