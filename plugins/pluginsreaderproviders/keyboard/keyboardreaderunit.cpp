...
std::vector<unsigned char> KeyboardReaderUnit::getChipInAir(unsigned int maxwait)
{
    ...
    if (process)
    {
        ...
        if (getKeyboardConfiguration()->getIsDecimalNumber())
        {
            ...
            for (std::vector<unsigned char>::iterator it = ret.begin(); it != ret.end();
                 ++it, --fact)
            {
                tmp[0]             = *it;
...
bool KeyboardReaderUnit::connectToReader()
{
    ...
    {
        std::strcpy(getKeyboardReaderProvider()->sKeyboard->selectedDeviceName,
                    getName().c_str());
        std::strcpy(getKeyboardReaderProvider()->sKeyboard->keyboardLayout,
