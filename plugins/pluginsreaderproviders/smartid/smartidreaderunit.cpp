    ...
    : ReaderUnit(READER_SMARTID)
{
    ...
    setDefaultReaderCardAdapter(
        std::shared_ptr<SmartIDReaderCardAdapter>(new SmartIDReaderCardAdapter()));
...
std::vector<unsigned char> SmartIDReaderUnit::getCardSerialNumber()
{
    ...
    std::vector<unsigned char> csn =
        getDefaultSmartIDReaderCardAdapter()->sendCommand(0x53, data);
    return csn;
}
...
{
    return BufferHelper::getStdString(getDefaultSmartIDReaderCardAdapter()->sendCommand(
        0x4F, std::vector<unsigned char>()));
}
