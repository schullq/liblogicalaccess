    ...
    : ReaderUnit(READER_GIGATMS)
{
    ...
    setDefaultReaderCardAdapter(
        std::shared_ptr<GigaTMSReaderCardAdapter>(new GigaTMSReaderCardAdapter()));
    ...
    try
    {
        ...
        read_xml(
            (boost::filesystem::current_path().string() + "/GigaTMSReaderUnit.config"),
            pt);
...
std::shared_ptr<Chip> GigaTMSReaderUnit::getChipInAir()
{
    ...
    std::vector<unsigned char> rawidbuf =
        getDefaultGigaTMSReaderCardAdapter()->sendCommand(cmd);
    if (rawidbuf.size() > 0)
    {
        ...
        if (rawidbuf[0] == 0x01)
        {
            std::string rawidstr = BufferHelper::getHex(
                std::vector<unsigned char>(rawidbuf.begin() + 1, rawidbuf.end()));
            std::vector<unsigned char> insertedId =
                XmlSerializable::formatHexString(rawidstr);
...
std::shared_ptr<GigaTMSReaderCardAdapter>
GigaTMSReaderUnit::getDefaultGigaTMSReaderCardAdapter()
{
