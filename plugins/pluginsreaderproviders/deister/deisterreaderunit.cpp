    ...
    : ReaderUnit(READER_DEISTER)
{
    ...
    setDefaultReaderCardAdapter(
        std::shared_ptr<DeisterReaderCardAdapter>(new DeisterReaderCardAdapter()));
    ...
    try
    {
        ...
        read_xml(
            (boost::filesystem::current_path().string() + "/DeisterReaderUnit.config"),
            pt);
    ...
    if (d_insertedChip)
    {
        ...
        do
        {
            std::this_thread::sleep_for(
                std::chrono::milliseconds(1000)); // Deister 'forget' the card...
            std::shared_ptr<Chip> chip = getChipInAir();
            if (chip)
            {
...
std::shared_ptr<Chip> DeisterReaderUnit::getChipInAir()
{
    ...
    std::vector<unsigned char> pollBuf =
        getDefaultDeisterReaderCardAdapter()->sendCommand(cmd);
    if (pollBuf.size() > 0)
    {
        std::vector<unsigned char> tmpId;
        if (pollBuf.size() > 0)
        {
            EXCEPTION_ASSERT_WITH_LOG(pollBuf[0] == 0x00, LibLogicalAccessException,
                                      "Bad polling answer, LOC byte");
            EXCEPTION_ASSERT_WITH_LOG(pollBuf[1] == 0x00, LibLogicalAccessException,
                                      "Bad polling answer, RST byte");
            EXCEPTION_ASSERT_WITH_LOG(pollBuf[2] == 0x01, LibLogicalAccessException,
                                      "Bad polling answer, NOB byte");
            // Only one card type supported at the same time for now
            std::string cardType =
                getCardTypeFromDeisterType(static_cast<DeisterCardType>(pollBuf[3]));
            EXCEPTION_ASSERT_WITH_LOG(pollBuf[4] == 0x01, LibLogicalAccessException,
                                      "Bad polling answer, DT byte");
            EXCEPTION_ASSERT_WITH_LOG(pollBuf[5] == 0x00, LibLogicalAccessException,
                                      "Bad polling answer, No byte");
            EXCEPTION_ASSERT_WITH_LOG(
                pollBuf[6] > 0, LibLogicalAccessException,
                "Bad polling answer, Size byte must be greater than zero");
            tmpId = std::vector<unsigned char>(pollBuf.begin() + 7,
                                               pollBuf.begin() + 7 + pollBuf[6]);
            std::reverse(tmpId.begin(), tmpId.end());
            chip = ReaderUnit::createChip(
                (d_card_type == CHIP_UNKNOWN ? cardType : d_card_type), tmpId);
        }
    }
...
std::string
DeisterReaderUnit::getCardTypeFromDeisterType(DeisterCardType deisterCardType) const
{
...
std::shared_ptr<DeisterReaderCardAdapter>
DeisterReaderUnit::getDefaultDeisterReaderCardAdapter()
{
