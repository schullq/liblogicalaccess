...
class LIBLOGICALACCESS_API PCSCReaderUnit : public ISO7816ReaderUnit
{
  public:
    ...
    /**
    * \brief Connect to the card.
    * \param reader The reader name. If an empty or invalid reader name is specified,
    * connect will fail.
    ...
    */
    ...
    static std::shared_ptr<PCSCReaderUnit>
    createPCSCReaderUnit(const std::string &readerName);

    /**
     * \brief Make the reader unit as a proxy to another. Use when listening on all PC/SC
     * reader unit.
     * \param readerUnit The referenced reader unit.
     * \param readerUnitConfig The referenced reader unit configuration.
     */
    void makeProxy(std::shared_ptr<PCSCReaderUnit> readerUnit,
    ...
    virtual void setLEDBuzzerDisplay(std::shared_ptr<LEDBuzzerDisplay> lbd) override;
