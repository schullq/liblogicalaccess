...
logicalaccess::PCSCConnection::PCSCConnection(PCSCShareMode mode, unsigned long protocol,
    ...
    , disposition(SCARD_LEAVE_CARD)
{
    ...
    LONG lReturn = SCardConnect(context, device.c_str(), mode, protocol, &handle_,
                                &active_protocol_);
...
{
    LONG lReturn =
        SCardReconnect(handle_, share_mode_, protocol_, disposition, &active_protocol_);
    if (lReturn != SCARD_S_SUCCESS)
    {
        THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException,
...
std::string logicalaccess::PCSCConnection::strerror(unsigned int error_flag)
{
    ...
    if (error_flag != SCARD_S_SUCCESS)
    {
        ...
        case SCARD_E_DUPLICATE_READER:
            msg += "The reader driver did not produce a unique reader name.";
            break;
        ...
        case SCARD_E_UNSUPPORTED_FEATURE:
            msg += "This smart card does not support the requested feature.";
            break;
        ...
        case SCARD_E_ICC_CREATEORDER:
            msg += "The requested order of object creation is not supported.";
            break;
        ...
        case SCARD_E_NO_FILE:
            msg += "The supplied path does not represent a smart card file.";
            break;
