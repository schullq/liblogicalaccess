...
MifareUltralightCPCSCCommands::sendGenericCommand(const std::vector<unsigned char> &data)
{
...
std::vector<unsigned char> MifareUltralightCPCSCCommands::authenticate_PICC2(
    const std::vector<unsigned char> &encRndAB)
{
    std::vector<unsigned char> data;
    data.push_back(0xAF);
...
void MifareUltralightCPCSCCommands::authenticate(std::shared_ptr<TripleDESKey> authkey)
{
    ...
    try
    {
        ...
        std::vector<unsigned char> key = std::vector<unsigned char>(
            authkey->getData(), authkey->getData() + authkey->getLength());
        std::vector<unsigned char> encRndB(result.begin() + 1, result.end() - 2);
        std::vector<unsigned char> rndB =
            DESFireCrypto::desfire_CBC_receive(key, iv, encRndB);
        ...
        std::vector<unsigned char> encRndAB =
            DESFireCrypto::sam_CBC_send(key, encRndB, rndAB);
        ...
        std::vector<unsigned char> rndA1 =
            DESFireCrypto::desfire_CBC_receive(key, iv, encRndA1);
