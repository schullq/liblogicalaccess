...
std::vector<unsigned char> ISO15693PCSCCommands::readBlock(size_t block, size_t le)
{
    ...
    result = getPCSCReaderCardAdapter()->sendAPDUCommand(0xff, 0xb0, p1, p2,
                                                         static_cast<unsigned char>(le));

    return std::vector<unsigned char>(result.begin(), result.end() - 2);
}

void ISO15693PCSCCommands::writeBlock(size_t block,
...
unsigned char ISO15693PCSCCommands::getSecurityStatus(size_t block)
{
    ...
    result = getPCSCReaderCardAdapter()->sendAPDUCommand(0xff, 0x30, 0x00, 0x03, 5,
                                                         command, 0x01);
