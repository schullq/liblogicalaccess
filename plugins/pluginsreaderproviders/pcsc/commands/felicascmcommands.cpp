...
FeliCaSCMCommands::requestServices(const std::vector<unsigned short> &codes)
{
    ...
    std::vector<unsigned char> result = getPCSCReaderCardAdapter()->sendAPDUCommand(
...
FeliCaSCMCommands::read(const std::vector<unsigned short> &codes,
                        const std::vector<unsigned short> &blocks)
{
    ...
    return std::vector<unsigned char>(result.begin() + 11,
                                      result.begin() + 11 + (nbBlocks * 16));
}
