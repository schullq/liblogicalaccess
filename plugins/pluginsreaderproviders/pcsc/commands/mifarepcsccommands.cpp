                                 ...
                                 std::shared_ptr<MifareKey> key, bool vol)
{
    ...
    std::vector<unsigned char> result,
        vector_key((unsigned char *)key->getData(),
...
std::vector<unsigned char> MifarePCSCCommands::readBinary(unsigned char blockno,
                                                          size_t len)
{
    TRACE(blockno, len);
    if (len >= 256)
    ...
    result = getPCSCReaderCardAdapter()->sendAPDUCommand(0xFF, 0xB0, 0x00, blockno,
                                                         static_cast<unsigned char>(len));
    return std::vector<unsigned char>(result.begin(), result.end() - 2);
}

void MifarePCSCCommands::updateBinary(unsigned char blockno,
