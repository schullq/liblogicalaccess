...
void ACSACR1222LLCDDisplay::setMessage(std::string message)
{
    ...
    for (unsigned char i = 0;
         i < 2 && (i * 15) < static_cast<unsigned char>(message.size()); i++)
    {
...
void ACSACR1222LLCDDisplay::scroll(unsigned char x, unsigned char y, unsigned char xRange,
                                   unsigned char yRange, unsigned char speedPixel,
                                   LCDScrollPeriod speedPeriod,
                                   LCDScrollDirection direction)
{
    std::vector<unsigned char> data;
    data.push_back(x);
    data.push_back(y);
...
{
    getPCSCReaderCardAdapter()->sendAPDUCommand(0xFF, 0x00, 0x64, (enable ? 0xff : 0x00),
                                                0x00);
}
}