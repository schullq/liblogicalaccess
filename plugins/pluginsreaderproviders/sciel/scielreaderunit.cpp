    ...
    : ReaderUnit(READER_SCIEL)
{
    ...
    setDefaultReaderCardAdapter(
        std::shared_ptr<SCIELReaderCardAdapter>(new SCIELReaderCardAdapter()));
    ...
    try
    {
        ...
        read_xml((boost::filesystem::current_path().string() + "/SCIELReaderUnit.config"),
                 pt);
...
bool SCIELReaderUnit::waitRemoval(unsigned int maxwait)
{
    ...
    if (d_insertedChip)
    {
        do
        {
            ...
            std::vector<std::shared_ptr<Chip>>::iterator i =
                std::find_if(chipList.begin(), chipList.end(),
                             SCIELReaderUnit::Finder(d_insertedChip));
            if (i == chipList.end())
            {
...
std::list<std::shared_ptr<Chip>> SCIELReaderUnit::getReaderChipList()
{
    ...
    try
    {
        ...
        std::list<std::vector<unsigned char>> allTags =
            getDefaultSCIELReaderCardAdapter()->receiveTagsListCommand(cmd);
        for (std::list<std::vector<unsigned char>>::iterator i = allTags.begin();
             i != allTags.end(); ++i)
        {
            LOG(LogLevel::INFOS) << "  -> allTags identifier "
                                 << BufferHelper::getHex((*i));
        }
...
/*
 * The timer "Time before tag out" is not used anymore: If tag was in tag-in and go in
 * tag-out, we keep the tag in the list (waitRemoval will still wait the tag go OUT of the
 * tag-out threshold)
 * We wait one or x tags in Tag-in, and we remove the tag from the list only if the tag is
 * not anymore in the tag-out threshold.
 */
std::list<std::shared_ptr<Chip>> SCIELReaderUnit::refreshChipList()
{
    ...
    for (std::list<std::shared_ptr<Chip>>::iterator i = chipList.begin();
         i != chipList.end(); ++i)
    {
        LOG(LogLevel::DEBUGS) << "  -> Chip identifier "
                              << BufferHelper::getHex((*i)->getChipIdentifier())
                              << " Reception {"
                              << static_cast<unsigned int>((*i)->getReceptionLevel())
                              << "}";
    }
    LOG(LogLevel::DEBUGS) << "**** END LIST ****";

    // Timer management. We handle the global timeout when tags disappears (it was done
    // before by the key, but it sucked !)
    for (std::vector<std::shared_ptr<Chip>>::iterator i = d_chipList.begin();
         i != d_chipList.end(); ++i)
    {
        bool findChip = false;
        for (std::list<std::shared_ptr<Chip>>::iterator j = chipList.begin();
             j != chipList.end() && !findChip; ++j)
        {
         ...
         i != chipList.end(); ++i)
    {
        ...
        if ((*i)->getReceptionLevel() <= getSCIELConfiguration()->getSafetyThreshold())
        {
            ...
            {
                safetyArea[identifierHexStr] =
                    getSCIELConfiguration()->getTimeBeforeSafety() / 1000;
        ...
        if (forceTagArea ||
            (*i)->getReceptionLevel() > getSCIELConfiguration()->getSafetyThreshold())
        {
            // We don't care about chip in tag-*** area if chip exists in safety area
            if (!safetyAreaUsed)
            {
                bool insertChip = false;
                if ((*i)->getReceptionLevel() <=
                    getSCIELConfiguration()->getTagInThreshold())
                {
                    if (getSCIELConfiguration()->getTagInThreshold() <
                        getSCIELConfiguration()->getTagOutThreshold())
                    {
                        ...
                        if (timeInArea <
                            (getSCIELConfiguration()->getTimeBeforeTagIn() / 1000))
                        {
                            tagInArea[identifierHexStr] = timeInArea;
                            if (d_safetyArea.find(identifierHexStr) != d_safetyArea.end())
                            {
                                safetyArea[identifierHexStr] =
                                    d_safetyArea[identifierHexStr];
                                d_chipList.push_back((*i));
                                safetyAreaUsed = true;
                            }
                        }
                    ...
                    else
                    {
                        ...
                        {
                            ignoreTag =
                                (d_ignoreArea[identifierHexStr] >=
                                 getSCIELConfiguration()->getTimeBeforeTagOut() / 1000);
                        }
                        ...
                        if (!ignoreTag)
                        {
                            ...
                            if ((*i)->getReceptionLevel() <=
                                getSCIELConfiguration()->getTagOutThreshold())
                            {
                                if (d_tagOutArea.find(identifierHexStr) !=
                                    d_tagOutArea.end())
                                {
                                ...
                                if (timeInArea <
                                    (getSCIELConfiguration()->getTimeBeforeTagOut() /
                                     1000))
                                {
                                    tagOutArea[identifierHexStr] = timeInArea;
                                    // The tag doesn't reach the time yet, we keep it in
                                    // last area
                                    if (d_safetyArea.find(identifierHexStr) !=
                                        d_safetyArea.end())
                                    {
                                        safetyArea[identifierHexStr] =
                                            d_safetyArea[identifierHexStr];
                                        if (safetyArea[identifierHexStr] >=
                                            getSCIELConfiguration()
                                                    ->getTimeBeforeSafety() /
                                                1000)
                                        {
                                            d_chipList.push_back((*i));
                                    ...
                                    else if (d_tagInArea.find(identifierHexStr) !=
                                             d_tagInArea.end())
                                    {
                                        tagInArea[identifierHexStr] =
                                            d_tagInArea[identifierHexStr];
                                        if (tagInArea[identifierHexStr] >=
                                            getSCIELConfiguration()
                                                    ->getTimeBeforeTagIn() /
                                                1000)
                                        {
                            ...
                            else
                            {
                                ...
                                if (d_safetyArea.find(identifierHexStr) !=
                                        d_safetyArea.end() ||
                                    d_tagOutArea.find(identifierHexStr) !=
                                        d_tagOutArea.end())
                                {
                                    if (d_ignoreArea.find(identifierHexStr) !=
                                        d_ignoreArea.end())
                                    {
                                    ...
                                    if (timeInArea <
                                        getSCIELConfiguration()->getTimeRemoval())
                                    {
                                        ignoreArea[identifierHexStr] = timeInArea;
                                        // The tag doesn't reach the time yet, we keep it
                                        // in last area
                                        if (d_safetyArea.find(identifierHexStr) !=
                                            d_safetyArea.end())
                                        {
                                        ...
                                        else if (d_tagOutArea.find(identifierHexStr) !=
                                                 d_tagOutArea.end())
                                        {
                                            tagOutArea[identifierHexStr] =
                                                d_tagOutArea[identifierHexStr];
                                            if (tagOutArea[identifierHexStr] >=
                                                getSCIELConfiguration()
                                                        ->getTimeBeforeTagOut() /
                                                    1000)
                                            {
                                ...
                                {
                                    if (d_tagInArea.find(identifierHexStr) !=
                                        d_tagInArea.end())
                                    {
                                        timeInArea += d_tagInArea[identifierHexStr];
                                    }
                                    ...
                                    if (timeInArea <
                                        (getSCIELConfiguration()->getTimeBeforeTagIn() /
                                         1000))
                                    {
                                        tagInArea[identifierHexStr] = timeInArea;
                                    }
                        ...
                        {
                            // Tag still present in tag-in area after tag-out, we continue
                            // to ignore it
                ...
                else if ((*i)->getReceptionLevel() <=
                         getSCIELConfiguration()->getTagOutThreshold())
                {
                    // Was already inserted
                    if (d_safetyArea.find(identifierHexStr) != d_safetyArea.end() ||
                        d_tagInArea.find(identifierHexStr) != d_tagInArea.end() ||
                        d_tagOutArea.find(identifierHexStr) != d_tagOutArea.end())
                    {
                ...
                if (!safetyAreaUsed)
                {
                    // We only insert chips which are recognized as inserted (after
                    // timeout management etc.)
                    if (insertChip)
                    {
                        d_chipList.push_back((*i));
                    }
                ...
                // Old functionnality :
                // Only one tag was selected before. Disabled because now we list all tags
                // that are in Tag-in or was in Tag-in and are now in Tag-out (so still
                // available).
                /*if (insertChip)
                {
    ...
    for (std::map<std::string, int>::iterator i = d_tagInArea.begin();
         i != d_tagInArea.end(); ++i)
    {
        LOG(LogLevel::DEBUGS) << "  -> Chip identifier {" << (*i).first
                              << "} Seconds in area {" << (*i).second << "}";
    }
    LOG(LogLevel::DEBUGS) << "**** END LIST ****";

    LOG(LogLevel::DEBUGS) << "**** TAG-OUT AREA CHIP LIST ****";
    for (std::map<std::string, int>::iterator i = d_tagOutArea.begin();
         i != d_tagOutArea.end(); ++i)
    {
        LOG(LogLevel::DEBUGS) << "  -> Chip identifier {" << (*i).first
                              << "} Seconds in area {" << (*i).second << "}";
    }
    ...
    for (std::map<std::string, int>::iterator i = d_safetyArea.begin();
         i != d_safetyArea.end(); ++i)
    {
        LOG(LogLevel::DEBUGS) << "  -> Chip identifier {" << (*i).first
                              << "} Seconds in area {" << (*i).second << "}";
    }
    ...
    for (std::map<std::string, int>::iterator i = d_ignoreArea.begin();
         i != d_ignoreArea.end(); ++i)
    {
        LOG(LogLevel::DEBUGS) << "  -> Chip identifier {" << (*i).first
                              << "} Seconds in area {" << (*i).second << "}";
    }
...
std::shared_ptr<SCIELReaderCardAdapter>
SCIELReaderUnit::getDefaultSCIELReaderCardAdapter()
{
...
bool SCIELReaderUnit::retrieveReaderIdentifier()
{
    ...
    try
    {
        ...
        std::vector<unsigned char> r =
            getDefaultSCIELReaderCardAdapter()->sendCommand(cmd);
        EXCEPTION_ASSERT_WITH_LOG(r.size() >= 2, LibLogicalAccessException,
                                  "Bad response getting SCIEL reader identifier.");
    ...
    LOG(LogLevel::INFOS) << "SCIEL Reader Identifier "
                         << BufferHelper::getHex(d_scielIdentifier) << " Len {"
                         << d_scielIdentifier.size() << "}...";
...
unsigned char SCIELReaderUnit::getTimeRemovalFromContextualStack()
{
    ...
    EXCEPTION_ASSERT_WITH_LOG(
        r.size() >= 6, LibLogicalAccessException,
        "Bad response getting SCIEL removal time. Bad response length.");
    EXCEPTION_ASSERT_WITH_LOG(
        r[0] == cmd[0] && r[1] == cmd[1], LibLogicalAccessException,
        "Bad response getting SCIEL removal time. Bad command response identifier.");
    EXCEPTION_ASSERT_WITH_LOG(
        r[4] == d_scielIdentifier[0] && r[5] == d_scielIdentifier[1],
        LibLogicalAccessException,
        "Bad response getting SCIEL removal time. Bad reader response identifier.");

    timeRemoval = static_cast<unsigned char>(strtoul(
        BufferHelper::getStdString(std::vector<unsigned char>(r.end() - 2, r.end()))
            .c_str(),
        NULL, 16));
...
void SCIELReaderUnit::setTimeRemovalFromContextualStack(unsigned char timeRemoval)
{
    ...
    std::vector<unsigned char> r = getDefaultSCIELReaderCardAdapter()->sendCommand(cmd);
    EXCEPTION_ASSERT_WITH_LOG(
        r.size() >= 8, LibLogicalAccessException,
        "Bad response setting SCIEL removal time. Bad response length.");
    EXCEPTION_ASSERT_WITH_LOG(
        r[0] == 'O' && r[1] == 'K', LibLogicalAccessException,
        "Bad response setting SCIEL removal time. Bad response status.");
    EXCEPTION_ASSERT_WITH_LOG(
        r[2] == cmd[0] && r[3] == cmd[1], LibLogicalAccessException,
        "Bad response setting SCIEL removal time. Bad command response identifier.");
    EXCEPTION_ASSERT_WITH_LOG(
        r[6] == d_scielIdentifier[0] && r[7] == d_scielIdentifier[1],
        LibLogicalAccessException,
        "Bad response setting SCIEL removal time. Bad reader response identifier.");
}
...
unsigned char SCIELReaderUnit::getReceptionLevel()
{
    ...
    EXCEPTION_ASSERT_WITH_LOG(
        r.size() >= 6, LibLogicalAccessException,
        "Bad response getting SCIEL reception level. Bad response length.");
    EXCEPTION_ASSERT_WITH_LOG(
        r[0] == cmd[0] && r[1] == cmd[1], LibLogicalAccessException,
        "Bad response getting SCIEL reception level. Bad command response identifier.");
    EXCEPTION_ASSERT_WITH_LOG(
        r[4] == d_scielIdentifier[0] && r[5] == d_scielIdentifier[1],
        LibLogicalAccessException,
        "Bad response getting SCIEL reception level. Bad reader response identifier.");

    level = static_cast<unsigned char>(strtoul(
        BufferHelper::getStdString(std::vector<unsigned char>(r.end() - 2, r.end()))
            .c_str(),
        NULL, 16));
...
void SCIELReaderUnit::setReceptionLevel(unsigned char level)
{
    ...
    EXCEPTION_ASSERT_WITH_LOG(
        r.size() >= 8, LibLogicalAccessException,
        "Bad response setting SCIEL reception level. Bad response length.");
    EXCEPTION_ASSERT_WITH_LOG(
        r[0] == 'O' && r[1] == 'K', LibLogicalAccessException,
        "Bad response setting SCIEL reception level. Bad response status.");
    EXCEPTION_ASSERT_WITH_LOG(
        r[2] == cmd[0] && r[3] == cmd[1], LibLogicalAccessException,
        "Bad response setting SCIEL reception level. Bad command response identifier.");
    EXCEPTION_ASSERT_WITH_LOG(
        r[6] == d_scielIdentifier[0] && r[7] == d_scielIdentifier[1],
        LibLogicalAccessException,
        "Bad response setting SCIEL reception level. Bad reader response identifier.");
}
...
unsigned char SCIELReaderUnit::getADConvertorValue()
{
    ...
    EXCEPTION_ASSERT_WITH_LOG(
        r.size() >= 6, LibLogicalAccessException,
        "Bad response getting SCIEL AD convertor value. Bad response length.");
    EXCEPTION_ASSERT_WITH_LOG(r[0] == cmd[0] && r[1] == cmd[1], LibLogicalAccessException,
                              "Bad response getting SCIEL AD convertor value. Bad "
                              "command response identifier.");
    EXCEPTION_ASSERT_WITH_LOG(
        r[4] == d_scielIdentifier[0] && r[5] == d_scielIdentifier[1],
        LibLogicalAccessException,
        "Bad response getting SCIEL AD convertor value. Bad reader response identifier.");

    convertorValue = static_cast<unsigned char>(strtoul(
        BufferHelper::getStdString(std::vector<unsigned char>(r.begin() + 2, r.end() - 2))
            .c_str(),
        NULL, 16));
...
std::shared_ptr<Chip>
SCIELReaderUnit::createChipFromBuffer(std::vector<unsigned char> buffer)
{
    ...
    if (buffer.size() > 2)
    {
        ...
        LOG(LogLevel::DEBUGS) << "Creating chip from buffer "
                              << BufferHelper::getHex(buffer) << " Len {" << buffer.size()
                              << "}...";
        ...
        else // Otherwise (32 bits), the power status cannot be used, the whole 32 bits is
             // the number !
        {
