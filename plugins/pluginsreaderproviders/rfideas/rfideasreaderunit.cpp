...
void RFIDeasReaderUnit::initExternFct()
{
    ...
    if ((fnGetLibVersion == NULL) || (fnSetComSrchRange == NULL) ||
        (fnCOMConnect == NULL) || (fnCOMDisconnect == NULL) || (fnUSBConnect == NULL) ||
        (fnUSBDisconnect == NULL) || (fnGetLastLibErr == NULL) || (fnReadCfg == NULL) ||
        (fnWriteCfg == NULL) || (fnGetFlags == NULL) || (fnSetFlags == NULL) ||
        (fnGetActiveID == NULL) || (fnGetActiveID32 == NULL) || (fnGetDevCnt == NULL) ||
        (fnSetActDev == NULL) || (fnSetLUID == NULL) || (fnGetQueuedID == NULL) ||
        (fnGetQueuedID_index == NULL) || (fnSetConnectProduct == NULL) ||
        (fnSetDevTypeSrch == NULL))
    {
        uninitExternFct();
