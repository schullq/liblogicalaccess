...
AxessTMC13ReaderCardAdapter::adaptCommand(const std::vector<unsigned char> &command)
{
    return command;
}

std::vector<unsigned char>
AxessTMC13ReaderCardAdapter::adaptAnswer(const std::vector<unsigned char> &answer)
{
    EXCEPTION_ASSERT_WITH_LOG(
        answer.size() > 0, std::invalid_argument,
        "A valid command buffer size must be at least 1 bytes long");
    EXCEPTION_ASSERT_WITH_LOG(answer[answer.size() - 1] == CR, std::invalid_argument,
                              "The supplied command buffer is not valid");
