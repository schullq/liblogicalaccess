    ...
    : ReaderUnit(READER_AXESSTMC13)
{
    ...
    setDefaultReaderCardAdapter(
        std::shared_ptr<AxessTMC13ReaderCardAdapter>(new AxessTMC13ReaderCardAdapter()));
    ...
    try
    {
        ...
        read_xml(
            (boost::filesystem::current_path().string() + "/AxessTMC13ReaderUnit.config"),
            pt);
...
std::shared_ptr<Chip> AxessTMC13ReaderUnit::getChipInAir()
{
    ...
    std::vector<unsigned char> tmpASCIIId =
        getDefaultAxessTMC13ReaderCardAdapter()->sendCommand(cmd);
...
std::shared_ptr<AxessTMC13ReaderCardAdapter>
AxessTMC13ReaderUnit::getDefaultAxessTMC13ReaderCardAdapter()
{
...
std::shared_ptr<AxessTMC13ReaderProvider>
AxessTMC13ReaderUnit::getAxessTMC13ReaderProvider() const
{
...
bool AxessTMC13ReaderUnit::retrieveReaderIdentifier()
{
    ...
    try
    {
        ...
        std::vector<unsigned char> r =
            getDefaultAxessTMC13ReaderCardAdapter()->sendCommand(cmd);
        EXCEPTION_ASSERT_WITH_LOG(r.size() >= 3, std::invalid_argument,
                                  "Bad response getting TMC reader identifier.");
        EXCEPTION_ASSERT_WITH_LOG(r[0] == 'T', std::invalid_argument,
                                  "Bad response getting TMC reader identifier, "
                                  "identifier byte 0 doesn't match.");
        EXCEPTION_ASSERT_WITH_LOG(r[1] == 'M', std::invalid_argument,
                                  "Bad response getting TMC reader identifier, "
                                  "identifier byte 1 doesn't match.");
        EXCEPTION_ASSERT_WITH_LOG(r[2] == 'C', std::invalid_argument,
                                  "Bad response getting TMC reader identifier, "
                                  "identifier byte 2 doesn't match.");
