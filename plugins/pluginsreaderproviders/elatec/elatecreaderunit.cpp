    ...
    : ReaderUnit(READER_ELATEC)
{
    ...
    setDefaultReaderCardAdapter(
        std::shared_ptr<ElatecReaderCardAdapter>(new ElatecReaderCardAdapter()));
...
std::shared_ptr<Chip> ElatecReaderUnit::getChipInAir()
{
    ...
    std::vector<unsigned char> result = getDefaultElatecReaderCardAdapter()->sendCommand(
        0x11, std::vector<unsigned char>());
    if (result.size() > 0)
    {
