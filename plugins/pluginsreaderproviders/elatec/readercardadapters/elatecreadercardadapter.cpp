...
ElatecReaderCardAdapter::adaptCommand(const std::vector<unsigned char> &command)
{
    EXCEPTION_ASSERT_WITH_LOG(command.size() >= 1, std::invalid_argument,
...
std::vector<unsigned char> ElatecReaderCardAdapter::sendCommand(
    unsigned char cmdcode, const std::vector<unsigned char> &command, long int timeout)
{
...
std::vector<unsigned char>
ElatecReaderCardAdapter::adaptAnswer(const std::vector<unsigned char> &answer)
{
    std::vector<unsigned char> ret;
    EXCEPTION_ASSERT_WITH_LOG(answer.size() >= 5, std::invalid_argument,
                              "No valid buffer recieved.");

    unsigned char buflength = answer[0];
    EXCEPTION_ASSERT_WITH_LOG(
        answer[2] == d_last_cmdcode, std::invalid_argument,
        "The request type doesn't match the command sent by the host");
    ...
    EXCEPTION_ASSERT_WITH_LOG(calcChecksum(std::vector<unsigned char>(
                                  answer.begin(), answer.begin() + buflength - 1)) ==
                                  checksum,
                              std::invalid_argument, "Wrong checksum");
