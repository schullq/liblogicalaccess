...
void SAMAV2ISO7816Commands::generateSessionKey(std::vector<unsigned char> rnda,
                                             ...
                                             unsigned char keyno)
{
    ...
    result = getISO7816ReaderCardAdapter()->sendAPDUCommand(d_cla, 0xa4, 0x00, 0x00, 0x03,
                                                            data_p1, 0x00);
    if (result.size() != 14 || result[12] != 0x90 || result[13] != 0xAF)
        THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException,
                                 "authenticateHost P1 Failed.");

    std::vector<unsigned char> keycipher(key->getData(),
    ...
    std::vector<unsigned char> macHost =
...
SAMAV2ISO7816Commands::createfullProtectionCmd(std::vector<unsigned char> cmd)
{
    bool lc, le;
    ...
    std::shared_ptr<openssl::OpenSSLSymmetricCipher> cipher(new openssl::AESCipher());
    std::vector<unsigned char> protectedCmd          = cmd, encProtectedCmd, cmdCtrVector,
                               finalFullProtectedCmd = cmd, encData;
    ...
    {
        std::vector<unsigned char> data(cmd.begin() + AV2_HEADER_LENGTH,
    ...
    if (lc)
    {
        ...
        std::vector<unsigned char> encMac(protectedCmd.begin(),
                                          protectedCmd.begin() + blockReady),
            tmp;
    ...
    encProtectedCmd = openssl::CMACCrypto::cmac(d_macSessionKey, cipher, 16, protectedCmd,
                                                d_lastMacIV, 16);
...
void SAMAV2ISO7816Commands::getLcLe(std::vector<unsigned char> cmd, bool &lc,
                                    unsigned char &lcvalue, bool &le)
{
...
std::vector<unsigned char>
SAMAV2ISO7816Commands::verifyAndDecryptResponse(std::vector<unsigned char> response)
{
    std::shared_ptr<openssl::SymmetricKey> symkeySession(new openssl::AESSymmetricKey(
        openssl::AESSymmetricKey::createFromData(d_sessionKey)));
    std::shared_ptr<openssl::SymmetricKey> symkeyMac(new openssl::AESSymmetricKey(
...
std::vector<unsigned char>
SAMAV2ISO7816Commands::dumpSecretKey(unsigned char keyno, unsigned char keyversion,
                                     std::vector<unsigned char> divInpu)
{
