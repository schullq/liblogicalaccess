...
void DESFireEV1ISO7816Commands::createApplication(
    ...
    std::vector<unsigned char> isoDFName)
{
...
void DESFireEV1ISO7816Commands::createLinearRecordFile(
    unsigned char fileno, EncryptionMode comSettings, DESFireAccessRights accessRights,
    unsigned int fileSize, unsigned int maxNumberOfRecords, unsigned short isoFID)
{
    std::vector<unsigned char> command;
    short ar = AccessRightsInMemory(accessRights);
...
void DESFireEV1ISO7816Commands::iso_selectFile(unsigned short fid)
{
    ...
    DESFireISO7816Commands::getISO7816ReaderCardAdapter()->sendAPDUCommand(
        DFEV1_CLA_ISO_COMPATIBLE, ISO7816_INS_SELECT_FILE, 0x00, 0x0C,
        static_cast<unsigned char>(data.size()), data);
}

std::vector<unsigned char>
DESFireEV1ISO7816Commands::iso_readRecords(unsigned short fid, unsigned char start_record,
                                           DESFireRecords record_number)
{
    ...
    result = DESFireISO7816Commands::getISO7816ReaderCardAdapter()->sendAPDUCommand(
        DFEV1_CLA_ISO_COMPATIBLE, ISO7816_INS_READ_RECORDS, p1, p2, 0x00);

    return std::vector<unsigned char>(result.begin(), result.end() - 2);
}

void DESFireEV1ISO7816Commands::iso_appendrecord(const std::vector<unsigned char> &data,
                                                 unsigned short fid)
{
...
std::vector<unsigned char>
DESFireEV1ISO7816Commands::iso_getChallenge(unsigned int length)
{
    std::vector<unsigned char> result;
...
void DESFireEV1ISO7816Commands::iso_externalAuthenticate(
...
std::vector<unsigned char> DESFireEV1ISO7816Commands::iso_internalAuthenticate(
    DESFireISOAlgorithm algorithm, bool isMasterCardKey, unsigned char keyno,
    const std::vector<unsigned char> &RPCD2, unsigned int length)
{
    std::vector<unsigned char> result;
                                                     ...
                                                     unsigned char keyno)
{
            ...
            key->getKeyDiversification()))
    {
        ...
        try
        {
            std::vector<unsigned char> randomAuthenticatePICC = {
    ...
    if (apduresult.size() <= 2 && apduresult[apduresult.size() - 2] != 0x90 &&
        ...
        THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException,
                                 "sam_iso_authenticate P1 failed.");

    std::vector<unsigned char> encRPCD1RPICC1(apduresult.begin(),
                                              apduresult.end() - 2 - 16);
    ...
    if (apduresult.size() <= 2 && apduresult[apduresult.size() - 2] != 0x90 &&
        ...
        THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException,
                                 "sam_iso_authenticate P2 failed.");
                                                 ...
                                                 unsigned char keyno)
{
    ...
    {
        iv.reset(new openssl::AESInitializationVector(
            openssl::AESInitializationVector::createFromData(std::vector<unsigned char>(
                cryptogram.end() - iv->data().size(), cryptogram.end()))));
    }
    else
    {
        iv.reset(new openssl::DESInitializationVector(
            openssl::DESInitializationVector::createFromData(std::vector<unsigned char>(
                cryptogram.end() - iv->data().size(), cryptogram.end()))));
    }
    ...
    if (cryptogram.size() < 1)
        THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException,
                                 "iso_authenticate wrong internal data.");

    std::vector<unsigned char> response;
    cipher->decipher(cryptogram, response, *isokey.get(), *iv.get(), false);

    std::vector<unsigned char> RPICC2 =
        std::vector<unsigned char>(response.begin(), response.begin() + le);
    std::vector<unsigned char> RPCD2a =
        std::vector<unsigned char>(response.begin() + le, response.end());

    EXCEPTION_ASSERT_WITH_LOG(RPCD2 == RPCD2a, LibLogicalAccessException,
    ...
    if (algorithm == DF_ALG_2K3DES)
    {
        if (std::vector<unsigned char>(keydiv.begin(), keydiv.begin() + 8) ==
            std::vector<unsigned char>(keydiv.begin() + 8, keydiv.begin() + 8 + 8))
        {
                                                ...
                                                DESFireISOAlgorithm algorithm)
{
    ...
    std::vector<unsigned char> encRndB =
        DESFireISO7816Commands::transmit(DFEV1_INS_AUTHENTICATE_ISO, data);
    ...
    std::vector<unsigned char> encRndA1 =
        DESFireISO7816Commands::transmit(DF_INS_ADDITIONAL_FRAME, response);
...
void DESFireEV1ISO7816Commands::authenticateAES(unsigned char keyno)
{
    ...
    std::vector<unsigned char> encRndB =
        DESFireISO7816Commands::transmit(DFEV1_INS_AUTHENTICATE_AES, data);
    unsigned char err = encRndB.back();
    encRndB.resize(encRndB.size() - 2);
    EXCEPTION_ASSERT_WITH_LOG(err == DF_INS_ADDITIONAL_FRAME, LibLogicalAccessException,
                              "No additional frame for ISO authentication.");
    ...
    std::vector<unsigned char> response =
        crypto->aes_authenticate_PICC1(keyno, diversify, encRndB);
    std::vector<unsigned char> encRndA1 =
        DESFireISO7816Commands::transmit(DF_INS_ADDITIONAL_FRAME, response);
    encRndA1.resize(encRndA1.size() - 2);
...
std::vector<unsigned char>
DESFireEV1ISO7816Commands::handleReadData(unsigned char err,
                                          const std::vector<unsigned char> &firstMsg,
                                                ...
                                                EncryptionMode mode)
{
    ...
    if (data.size() <= DESFIRE_EV1_CLEAR_DATA_LENGTH_CHUNK)
    {
        ...
        case CM_PLAIN:
        {
            ...
            if (crypto->d_auth_method != CM_LEGACY) // CMAC needs to be recalculated
            {
                ...
                if (parameters != NULL)
                {
                    apdu_command.insert(apdu_command.end(), parameters,
                                        parameters + paramLength);
                }
                if (data.size() > 0)
                {
        ...
        case CM_ENCRYPT:
        {
            ...
            if (parameters != NULL)
            {
                encparameters.insert(encparameters.end(), parameters,
                                     parameters + paramLength);
            }
    ...
    else
    {
        ...
        {
        case CM_PLAIN:
            edata = std::vector<unsigned char>(
                data.begin(), data.begin() + DESFIRE_EV1_CLEAR_DATA_LENGTH_CHUNK);
            if (crypto->d_auth_method != CM_LEGACY) // CMAC needs to be recalculated
            {
                std::vector<unsigned char> apdu_command;
                apdu_command.push_back(cmd);
        ...
        case CM_MAC:
            ...
            else
            {
                ...
                if (parameters != NULL)
                {
                    gdata.insert(gdata.end(), (unsigned char *)parameters,
                                 (unsigned char *)parameters + paramLength);
                }
    ...
    if (data.size() > DESFIRE_EV1_CLEAR_DATA_LENGTH_CHUNK)
    {
        ...
        {
            size_t pkSize = ((data.size() - pos) >= (DESFIRE_EV1_CLEAR_DATA_LENGTH_CHUNK))
            ...
            {
            case CM_PLAIN:
                edata = std::vector<unsigned char>(data.begin() + pos,
                                                   data.begin() + pos + pkSize);
                break;
            ...
            case CM_ENCRYPT:
                if (pos + pkSize == data.size())
                {
                    ...
                    if (parameters != NULL)
                    {
                        encparameters.insert(encparameters.end(),
                                             (unsigned char *)parameters,
                                             (unsigned char *)parameters + paramLength);
                    }
                    edata = crypto->encipherData(
                        true, std::vector<unsigned char>(data.begin() + pos,
                                                         data.begin() + pos + pkSize),
                        encparameters);
                }
                else
                {
                    edata = crypto->encipherData(
                        false, std::vector<unsigned char>(data.begin() + pos,
                                                          data.begin() + pos + pkSize));
                }
                break;
    ...
    if (err == 0x00 && result.size() > 2 &&
        (mode == CM_MAC || mode == CM_ENCRYPT || crypto->d_auth_method != CM_LEGACY))
    {
        result.resize(result.size() - 2);
        if (crypto->verifyMAC(true, result))
        {
            // THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException, "MAC verification
            // failed.");
        }
    }
}

std::vector<unsigned char> DESFireEV1ISO7816Commands::readData(unsigned char fileno,
                                                               unsigned int offset,
                                                               unsigned int length,
                                                               EncryptionMode mode)
{
...
std::vector<unsigned char> DESFireEV1ISO7816Commands::readRecords(unsigned char fileno,
                                                                  unsigned int offset,
                                                                  unsigned int length,
                                                                  EncryptionMode mode)
{
...
void DESFireEV1ISO7816Commands::changeFileSettings(unsigned char fileno,
                                                   EncryptionMode comSettings,
                                                   DESFireAccessRights accessRights,
                                                   bool plain)
{
...
void DESFireEV1ISO7816Commands::changeKeySettings(DESFireKeySettings settings)
{
    ...
    std::vector<unsigned char> cryptogram = crypto->desfireEncrypt(
        std::vector<unsigned char>(command, command + sizeof(command)),
        std::vector<unsigned char>(param, param + sizeof(param)));
    ...
    else
    {
        ...
        if (dd.size() > 0 && keyno != 0)
        {
            std::vector<unsigned char> CMAC =
                crypto->desfire_cmac(std::vector<unsigned char>(&err, &err + 1));
            EXCEPTION_ASSERT_WITH_LOG(dd == CMAC, LibLogicalAccessException,
                                      "Wrong CMAC.");
        }
    }
}

void DESFireEV1ISO7816Commands::getVersion(
                                                 ...
                                                 bool randomIdEnabled)
{
    ...
    // command[0] = 0x00;
    command[0] =
        0x00 | ((formatCardEnabled) ? 0x00 : 0x01) | ((randomIdEnabled) ? 0x02 : 0x00);

    crypto->initBuf(sizeof(command));
    std::vector<unsigned char> encBuffer = crypto->desfire_encrypt(
        crypto->d_sessionKey,
        std::vector<unsigned char>(command, command + sizeof(command)));
...
void DESFireEV1ISO7816Commands::setConfiguration(std::shared_ptr<DESFireKey> defaultKey)
{
    ...
    std::vector<unsigned char> encBuffer = crypto->desfireEncrypt(
        std::vector<unsigned char>(command, command + sizeof(command)));
    std::vector<unsigned char> buf;
    buf.push_back(0x02);
    buf.insert(buf.end(), encBuffer.begin(), encBuffer.end());
...
std::vector<unsigned char>
DESFireEV1ISO7816Commands::transmit(unsigned char cmd,
                                    ...
                                    unsigned char lc, bool forceLc)
{
    ...
    std::vector<unsigned char> r =
        DESFireISO7816Commands::transmit(cmd, buf, lc, forceLc);
    unsigned char err = r.back();
    if (crypto->d_auth_method != CM_LEGACY && cmd != DF_INS_ADDITIONAL_FRAME &&
        r.size() > 8 && err != DF_INS_ADDITIONAL_FRAME)
    {
        std::vector<unsigned char> response =
            std::vector<unsigned char>(r.begin(), r.end() - 10);
        response.push_back(err);

        CMAC = crypto->desfire_cmac(response);

        EXCEPTION_ASSERT_WITH_LOG(std::vector<unsigned char>(r.end() - 10, r.end() - 2) ==
                                      CMAC,
...
std::vector<unsigned char> DESFireEV1ISO7816Commands::transmit_plain(unsigned char cmd,
                                                                     unsigned char lc)
{
...
std::vector<unsigned char>
DESFireEV1ISO7816Commands::transmit_plain(unsigned char cmd,
                                          const std::vector<unsigned char> &buf,
                                          unsigned char lc, bool forceLc)
{
...
std::vector<unsigned char> DESFireEV1ISO7816Commands::transmit_nomacv(unsigned char cmd,
                                                                      unsigned char lc)
{
...
std::vector<unsigned char>
DESFireEV1ISO7816Commands::transmit_nomacv(unsigned char cmd,
                                           const std::vector<unsigned char> &buf,
                                           unsigned char lc, bool forceLc)
{
    std::shared_ptr<DESFireCrypto> crypto = getDESFireChip()->getCrypto();
