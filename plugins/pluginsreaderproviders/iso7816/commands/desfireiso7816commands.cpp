                                                  ...
                                                  std::shared_ptr<DESFireKey> key)
{
    ...
    std::shared_ptr<DESFireKey> oldkey    = std::dynamic_pointer_cast<DESFireKey>(
        crypto->getKey(crypto->d_currentAid, keyno));
        ...
        std::dynamic_pointer_cast<NXPKeyDiversification>(oldkey->getKeyDiversification()))
    {
        ...
        if (oldnxpdiv)
        {
            ...
            if (nxpdiv &&
                !std::equal(diversifyNew.begin(), diversifyNew.end(),
                            diversifyOld.begin()) &&
...
void DESFireISO7816Commands::changeKeySettings(DESFireKeySettings settings)
{
    ...
    std::vector<unsigned char> cryptogram = getDESFireChip()->getCrypto()->desfireEncrypt(
        std::vector<unsigned char>(command, command + sizeof(command)));
    transmit(DF_INS_CHANGE_KEY_SETTINGS, cryptogram);
}

void DESFireISO7816Commands::getFileSettings(unsigned char fileno,
...
std::vector<unsigned char>
DESFireISO7816Commands::handleReadData(unsigned char err,
...
void DESFireISO7816Commands::handleWriteData(unsigned char cmd, unsigned char *parameters,
                                             ...
                                             EncryptionMode mode)
{
    ...
    else
    {
        ...
        case CM_PLAIN:
            edata = std::vector<unsigned char>(
                data.begin(), data.begin() + DESFIRE_CLEAR_DATA_LENGTH_CHUNK);
            break;
        ...
        case CM_ENCRYPT:
            edata = crypto->encipherData(
                false, std::vector<unsigned char>(
                           data.begin(), data.begin() + DESFIRE_CLEAR_DATA_LENGTH_CHUNK));
            break;
    ...
    if (data.size() > DESFIRE_CLEAR_DATA_LENGTH_CHUNK)
    {
        ...
        {
            size_t pkSize = ((data.size() - pos) >= DESFIRE_CLEAR_DATA_LENGTH_CHUNK)
...
void DESFireISO7816Commands::createLinearRecordFile(unsigned char fileno,
                                                    EncryptionMode comSettings,
                                                    DESFireAccessRights accessRights,
                                                    unsigned int fileSize,
...
void DESFireISO7816Commands::createCyclicRecordFile(unsigned char fileno,
                                                    EncryptionMode comSettings,
                                                    DESFireAccessRights accessRights,
                                                    unsigned int fileSize,
                                                    unsigned int maxNumberOfRecords)
...
std::vector<unsigned char> DESFireISO7816Commands::readData(unsigned char fileno,
                                                            unsigned int offset,
                                                            unsigned int length,
                                                            EncryptionMode mode)
{
                                       ...
                                       EncryptionMode mode)
{
    ...
    handleWriteData(DF_INS_WRITE_DATA, parameters,
                    static_cast<unsigned int>(sizeof(parameters)), data, mode);
}
                                    ...
                                    EncryptionMode mode)
{
    ...
    handleWriteData(DF_INS_CREDIT, parameters,
                    static_cast<unsigned int>(sizeof(parameters)), data, mode);
}
                                   ...
                                   EncryptionMode mode)
{
    ...
    std::vector<unsigned char> data(sizeof(dataValue));
    memcpy(&data[0], &dataValue, sizeof(dataValue));
    handleWriteData(DF_INS_DEBIT, parameters,
                    static_cast<unsigned int>(sizeof(parameters)), data, mode);
}
                                           ...
                                           EncryptionMode mode)
{
    ...
    std::vector<unsigned char> data(sizeof(dataValue));
    memcpy(&data[0], &dataValue, sizeof(dataValue));
    handleWriteData(DF_INS_LIMITED_CREDIT, parameters,
                    static_cast<unsigned int>(sizeof(parameters)), data, mode);
}
                                         ...
                                         EncryptionMode mode)
{
    ...
    handleWriteData(DF_INS_WRITE_RECORD, parameters,
                    static_cast<unsigned int>(sizeof(parameters)), data, mode);
}

std::vector<unsigned char> DESFireISO7816Commands::readRecords(unsigned char fileno,
                                                               unsigned int offset,
                                                               unsigned int length,
                                                               EncryptionMode mode)
{
                                           ...
                                           std::vector<unsigned char> diversify)
{
    ...
    if (key->getKeyDiversification() &&
        ...
        THROW_EXCEPTION_WITH_LOG(
            LibLogicalAccessException,
            "SAM AV2 can only dump key with diversification NXP AV2.");
    key->setData(
                                          ...
                                          std::shared_ptr<DESFireKey> currentKey)
{
    ...
    std::vector<unsigned char> result =
        DESFireISO7816Commands::transmit(DF_INS_AUTHENTICATE, command);
        ...
        (result.size() - 2) >= 8)
    {
        ...
        if (samKeyStorage && !samKeyStorage->getDumpKey())
        {
            ...
            data[0] = std::dynamic_pointer_cast<SAMKeyStorage>(key->getKeyStorage())
                          ->getKeySlot();
            data[1] = key->getKeyVersion();
            data.insert(data.end(), result.begin(), result.end());

            if (std::dynamic_pointer_cast<NXPKeyDiversification>(
                    key->getKeyDiversification()))
            {
                p1 |= 0x01;
            ...
            do
            {
                ...
                {
                    if (trytoreconnect > 5 ||
                        ex.error_code() != CardException::WRONG_P1_P2 ||
                    ...
                    // diversification av2
                    LOG(LogLevel::WARNINGS) << "try to auth with SAM P1: "
                                            << trytoreconnect;
                    getSAMChip()
                        ->getCommands()
            ...
            if (apduresult.size() <= 2)
                THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException,
                                         "sam authenticate DES P1 failed.");
            ...
            if (samKeyStorage && !samKeyStorage->getDumpKey())
            {
                ...
                if (getSAMChip()->getCardType() == "SAM_AV1")
                    ...
                    apduresult = std::dynamic_pointer_cast<
                                     ...
                                     getSAMChip()->getCommands())
                                     ->transmit(cmd_vector, true, false);
                if (apduresult.size() != 2 || apduresult[0] != 0x90 ||
                    apduresult[1] != 0x00)
                    THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException,
                                             "sam authenticate DES P2 failed.");
        ...
        THROW_EXCEPTION_WITH_LOG(LibLogicalAccessException,
                                 "DESFire authentication P1 failed.");
}
...
std::vector<unsigned char>
DESFireISO7816Commands::transmit(unsigned char cmd,
                                 const std::vector<unsigned char> &data, unsigned char lc,
                                 bool forceLc)
{
    if (data.size())
    {
                                                  ...
                                                  std::shared_ptr<DESFireKey> key)
{
    ...
    if (cmd.flag_ & IKS_COMMAND_DESFIRE_CHANGEKEY_OTHER_KEY)
        crypto->d_lastIV =
            std::vector<unsigned char>(resp->bytes_.end() - 16, resp->bytes_.end());
    return resp->bytes_;
}
}
