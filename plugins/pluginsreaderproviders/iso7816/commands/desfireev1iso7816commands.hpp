                                                       ...
                                                       public ISO7816ISO7816Commands
{
  public:
    ...
    virtual void createApplication(
        ...
        std::vector<unsigned char> isoDFName = std::vector<unsigned char>());
    ...
    virtual void createBackupFile(unsigned char fileno, EncryptionMode comSettings,
                                  DESFireAccessRights accessRights, unsigned int fileSize,
    ...
    iso_readRecords(unsigned short fid = 0, unsigned char start_record = 0,
                    DESFireRecords record_number = DF_RECORD_ONERECORD);
     ...
     * \param data The data.
     */
    virtual void iso_externalAuthenticate(
        DESFireISOAlgorithm algorithm = DF_ALG_BY_CONTEXT, bool isMasterCardKey = true,
     ...
     */
    virtual std::vector<unsigned char> iso_internalAuthenticate(
        DESFireISOAlgorithm algorithm = DF_ALG_BY_CONTEXT, bool isMasterCardKey = true,
        unsigned char keyno                     = 0x00,
        const std::vector<unsigned char> &RPCD2 = std::vector<unsigned char>(),
     ...
     */
    virtual std::vector<unsigned char> readData(unsigned char fileno, unsigned int offset,
     ...
     */
    virtual std::vector<unsigned char> readRecords(unsigned char fileno,
                                                   unsigned int offset,
                                                   unsigned int length,
                                                   EncryptionMode mode);
    ...
    virtual void changeFileSettings(unsigned char fileno, EncryptionMode comSettings,
                                    DESFireAccessRights accessRights, bool plain);
    ...
    virtual void iso_selectApplication(
        std::vector<unsigned char> isoaid = std::vector<unsigned char>(
            DFEV1_DESFIRE_AID, DFEV1_DESFIRE_AID + sizeof(DFEV1_DESFIRE_AID)));
  ...
  protected:
     ...
     */
    virtual std::vector<unsigned char>
    handleReadData(unsigned char err, const std::vector<unsigned char> &firstMsg,
                   unsigned int length, EncryptionMode mode);
     ...
     * \param mode The communication mode
     */
    virtual void handleWriteData(unsigned char cmd, unsigned char *parameters,
     ...
     */
    virtual std::vector<unsigned char>
    transmit(unsigned char cmd,
             const std::vector<unsigned char> &buf = std::vector<unsigned char>(),
     ...
     * \return The response.
     */
    virtual std::vector<unsigned char> transmit_plain(unsigned char cmd,
                                                      unsigned char lc);
     ...
     */
    virtual std::vector<unsigned char>
    transmit_plain(unsigned char cmd,
                   const std::vector<unsigned char> &buf = std::vector<unsigned char>(),
                   unsigned char lc = 0, bool forceLc = false);
     ...
     * \return The response.
     */
    virtual std::vector<unsigned char> transmit_nomacv(unsigned char cmd,
                                                       unsigned char lc);
     ...
     */
    virtual std::vector<unsigned char>
    transmit_nomacv(unsigned char cmd,
                    const std::vector<unsigned char> &buf = std::vector<unsigned char>(),
                    unsigned char lc = 0, bool forceLc = false);
