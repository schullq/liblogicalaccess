...
class LIBLOGICALACCESS_API DESFireISO7816Commands : public virtual DESFireCommands
{
  public:
    ...
    */
    void getKeyFromSAM(std::shared_ptr<DESFireKey> key,
                       std::vector<unsigned char> diversify);

  protected:
    std::vector<unsigned char> getChangeKeySAMCryptogram(unsigned char keyno,
                                                         std::shared_ptr<DESFireKey> key);

    std::vector<unsigned char> getChangeKeyIKSCryptogram(unsigned char keyno,
                                                         std::shared_ptr<DESFireKey> key);
