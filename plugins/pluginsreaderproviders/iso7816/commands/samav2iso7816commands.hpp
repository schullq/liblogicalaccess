      ...
      public SAMAV2Commands<KeyEntryAV2Information, SETAV2>
{
  public:
    ...
    virtual std::shared_ptr<SAMKeyEntry<KeyEntryAV2Information, SETAV2>>
    getKeyEntry(unsigned char keyno);
    ...
    virtual std::vector<unsigned char> transmit(std::vector<unsigned char> cmd,
                                                bool first = true, bool last = true);

    virtual std::vector<unsigned char> dumpSecretKey(unsigned char keyno,
                                                     unsigned char keyversion,
                                                     std::vector<unsigned char> divInpu);

  protected:
    void generateSessionKey(std::vector<unsigned char> rnd1,
                            std::vector<unsigned char> rnd2);

    std::vector<unsigned char> createfullProtectionCmd(std::vector<unsigned char> cmd);

    std::vector<unsigned char>
    verifyAndDecryptResponse(std::vector<unsigned char> response);

    void getLcLe(std::vector<unsigned char> cmd, bool &lc, unsigned char &lcvalue,
                 bool &le);
