...
void TwicISO7816Commands::selectTWICApplication()
{
    ...
    try
    {
        ...
        getISO7816ReaderCardAdapter()->sendAPDUCommand(
            0x00, 0xA4, 0x04, 0x00, sizeof(command), command, sizeof(command));
    }
    catch (std::exception &)
    {
        // Gouvernemental card
...
std::vector<unsigned char> TwicISO7816Commands::getTWICData(int64_t dataObject)
{
    ...
    result = getISO7816ReaderCardAdapter()->sendAPDUCommand(
        0x00, 0xCB, 0x3F, 0xFF, static_cast<unsigned char>(command.size()), command,
        static_cast<unsigned char>(command.size()));
    if (result.size() != 2)
    {
