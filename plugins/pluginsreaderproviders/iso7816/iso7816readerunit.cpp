...
bool ISO7816ReaderUnit::reconnect(int action)
{
    try
    {
        ...
        if (chip && chip->getGenericCardType() == CHIP_SAM)
        {
            ...
            auto command = std::dynamic_pointer_cast<logicalaccess::SAMCommands<
                logicalaccess::KeyEntryAV2Information, logicalaccess::SETAV2>>(
                chip->getCommands());
            if (command)
                command->lockUnlock(getISO7816Configuration()->getSAMUnLockKey(),
                                    logicalaccess::SAMLockUnlock::Unlock, 0, 0, 0);
        }
    }
            ...
            else if (ret->getSingleChip()->getCardType() == "SAM_AV2")
            {
                try
                {
                    std::dynamic_pointer_cast<SAMAV2ISO7816Commands>(
                        getSAMChip()->getCommands())
                        ->lockUnlock(getISO7816Configuration()->getSAMUnLockKey(),
                                     logicalaccess::SAMLockUnlock::Unlock,
                ...
                catch (CardException &ex)
                {
                    ...
                    std::dynamic_pointer_cast<SAMAV2ISO7816Commands>(
                        ...
                        ->lockUnlock(
                            ...
                            logicalaccess::SAMLockUnlock::LockWithoutSpecifyingKey,
