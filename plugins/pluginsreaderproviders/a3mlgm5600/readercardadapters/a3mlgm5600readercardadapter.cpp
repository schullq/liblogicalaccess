...
A3MLGM5600ReaderCardAdapter::adaptCommand(const std::vector<unsigned char> &data)
{
...
std::vector<unsigned char> A3MLGM5600ReaderCardAdapter::sendCommand(
    unsigned char cmd, const std::vector<unsigned char> &data, long int timeout)
{
...
std::vector<unsigned char>
A3MLGM5600ReaderCardAdapter::adaptAnswer(const std::vector<unsigned char> &answer)
{
    ...
    std::vector<unsigned char> buf =
        std::vector<unsigned char>(answer.begin() + 1, answer.end());
        ...
        if (d_command_status == 0x46)
        {
            ...
            if (buf.size() == 5)
            {
                ...
                if (d_command_status == 0x46)
                {
                    ...
                    {
                        partuid =
                            std::vector<unsigned char>(buf.begin(), buf.begin() + 4);
                        fulluid.insert(fulluid.end(), partuid.begin(), partuid.end());
...
std::vector<unsigned char>
A3MLGM5600ReaderCardAdapter::anticollisionB(unsigned char /*afi*/)
{
    /* TODO: Not implemented yet */
    return std::vector<unsigned char>();
}
