    ...
    : ReaderUnit(READER_A3MLGM5600)
{
    ...
    setDefaultReaderCardAdapter(
        std::shared_ptr<A3MLGM5600ReaderCardAdapter>(new A3MLGM5600ReaderCardAdapter()));
    ...
    try
    {
        ...
        read_xml(
            (boost::filesystem::current_path().string() + "/A3MLGM5600ReaderUnit.config"),
            pt);
...
string A3MLGM5600ReaderUnit::getPADKey()
{
    ...
    std::vector<unsigned char> res =
        adapter->sendCommand(0x17, std::vector<unsigned char>());
...
string A3MLGM5600ReaderUnit::getReaderSerialNumber()
{
    std::shared_ptr<A3MLGM5600ReaderCardAdapter> adapter =
        getDefaultA3MLGM5600ReaderCardAdapter();
    std::vector<unsigned char> res =
        adapter->sendCommand(0x09, std::vector<unsigned char>());
